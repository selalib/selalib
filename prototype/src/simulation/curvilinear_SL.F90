...
#include "sll_memory.h"
  ...
  ! 1 : f=1
  ! 2 : cos eta2
  ! 3 : centered gaussian in eta1 and eta2 (mesh_case = 1,4)
  ...
  ! 2 : rotation
  ! 3 : divergence free non-symmetric field (mesh_case = 1)
  ! 4 : divergence free complex field (mesh_case = 2,3)
  field_case = 1
  if (field_case > 4) then
    print*,'Non existing case'
    print*,'field_case = 1 (transaltion), 2 (rotation),'
    print*,'3 (divergence free non-symmetric field for cartesian mesh),' 
    print*,'or 4 (complex divergence free field for polar/polar-like mesh)'
    STOP
  end if
  !a1=0.001_f64
  !a2=0.002_f64
  ...
  !dt = 0.1_f64*delta_eta1 
  !nb_step = floor(0.1_f64/dt
  
  dt = 0.01_f64*delta_eta1
  nb_step = floor(0.01_f64/dt)
      ...
      ! Displacement
        ...
        if (field_case==2) then
          ...
          if (mesh_case==4) then
            ...
            x2t = eta2 + alpha_mesh*sin(2*sll_pi*eta1)*sin(2*sll_pi*eta2)
            if ((abs(x1-x1t)/max(x1,x1t)>1E-10).or.(abs(x2-x2t)/max(x2,x2t)>1E-10)) then
              print*,'problem with Newton solver in the analytic part with the rotation - analytic solution', &
                  abs(x1-x1t)/max(x1,x1t), abs(x2-x2t)/max(x2,x2t), x1, x1t, x2, x2t
              STOP
            end if
          end if
        ...
        ! divergence free complex fields
        ! solved with RK4
        if (field_case==3) then
          eta1c = 0.5_f64*(eta1_max+eta1_min)
          eta2c = 0.5_f64*(eta2_max+eta2_min)
          eta1t = eta1c+(eta1-eta1c)*cos(sqrt(a1*a2)*dt) - (eta2-eta2c)*sqrt(a2/a1)*sin(sqrt(a1*a2)*dt)
          eta2t = eta2c+(eta1-eta1c)*sqrt(a1/a2)*sin(sqrt(a1*a2)*dt) + (eta2-eta2c)*cos(sqrt(a1*a2)*dt)
          eta1 = eta1t
          eta2 = eta2t
        end if
        if (field_case==4) then
        ...
        if (field_case==1) then
          ...
          if (mesh_case==4) then
            ...
            x2t = eta2 + alpha_mesh*sin(2*sll_pi*eta1)*sin(2*sll_pi*eta2)
            if ((abs(x1-x1t)/max(x1,x1t)>1E-10).or.(abs(x2-x2t)/max(x2,x2t)>1E-10)) then
              print*,'problem with Newton solver in the analytic part with the translation - BSL', &
                  abs(x1-x1t)/max(x1,x1t), abs(x2-x2t)/max(x2,x2t), x1, x1t, x2, x2t
              STOP
            end if
          end if
        ...
        if (field_case==2) then
          ...
          if (mesh_case==4) then
            ...
            x2t = eta2 + alpha_mesh*sin(2*sll_pi*eta1)*sin(2*sll_pi*eta2)
            if ((abs(x1-x1t)/max(x1,x1t)>1E-10).or.(abs(x2-x2t)/max(x2,x2t)>1E-10)) then
              print*,'problem with Newton solver in the analytic part with the rotation', &
                  abs(x1-x1t)/max(x1,x1t), abs(x2-x2t)/max(x2,x2t), x1, x1t, x2, x2t
              STOP
            end if
          end if
        ...
        if (field_case==1) then
          ...
          if (mesh_case==4) then
            ...
            x2t = eta2 + alpha_mesh*sin(2*sll_pi*eta1)*sin(2*sll_pi*eta2)
            if ((abs(x1-x1t)/max(x1,x1t)>1E-10).or.(abs(x2-x2t)/max(x2,x2t)>1E-10)) then
              print*,'problem with Newton solver in the analytic part with the translation - FSL', &
                  abs(x1-x1t)/max(x1,x1t), abs(x2-x2t)/max(x2,x2t), x1, x1t, x2, x2t
              STOP
            end if
          end if
        ...
        if (field_case==2) then
          ...
          if (mesh_case==4) then
            ...
            x2t = eta2 + alpha_mesh*sin(2*sll_pi*eta1)*sin(2*sll_pi*eta2)
            if ((abs(x1-x1t)/max(x1,x1t)>1E-10).or.(abs(x2-x2t)/max(x2,x2t)>1E-10)) then
              print*,'problem with Newton solver in the analytic part with the rotation - FSL', &
                  abs(x1-x1t)/max(x1,x1t), abs(x2-x2t)/max(x2,x2t), x1, x1t, x2, x2t
              STOP
            end if
          end if

!**************************************************************
!  This module defines an hexagonal mesh subdivided in
!  equilateral triangles. The mesh is generated by three
...
#define TEST_PRESENCE_AND_ASSIGN_VAL( obj, arg, slot, default_val ) \
  ...
  function from_cart_index_k1(mesh, x1, x2) result(k1)
    ...
    ! If we have the matrix R = (r1, r2) and the vectors k=(k1, k2), x=(x1, x2)
    ! Then the coordinates transformation yields : Rk = x
    ! to find k, we inverse the 2x2 transformation matrix R
    ! we obtain : R^(-1)x = k

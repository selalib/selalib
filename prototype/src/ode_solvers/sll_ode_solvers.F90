...
contains
  ...
  subroutine implicit_ode_nonuniform_new( order,  &
       deltat, &
       xin,   &
       ncx,    &
       bt,     &
    ...
    if (bt == PERIODIC_ODE) then
      ...
      !end modif
       ...
       i0p1=i0+1
       yi0p1=xin(i0p1) + c*deltat*( a(i0p1) + b(i) )
       do i = 1, ncx
          ...
          do while(xi>=yi0p1)
            ...
            yi0=yi0p1
            if(i0p1>ncx)then
              yi0p1=xin(i0p1-ncx)+period + c*deltat*( a(i0p1-ncx) + b(i) )
            endif
  ...
  end subroutine implicit_ode_nonuniform_new

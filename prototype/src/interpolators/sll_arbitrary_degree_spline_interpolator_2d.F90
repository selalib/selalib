...
if (bc_w == SLL_DIRICHLET .and. present(value_w)) then
  ...
  interpolator%value_w(1:sz_value_w) = &
    interpolator%interp1d_w%bcoef(1:sz_value_w)
...
if (bc_e == SLL_DIRICHLET .and. present(value_e)) then
  ...
  interpolator%value_e(1:sz_value_e) = &
    interpolator%interp1d_e%bcoef(1:sz_value_e)
...
if (bc_s == SLL_DIRICHLET .and. present(value_s)) then
  ...
  interpolator%value_s(1:sz_value_s) = &
    interpolator%interp1d_s%bcoef(1:sz_value_s)
...
if (bc_n == SLL_DIRICHLET .and. present(value_n)) then
  ...
  interpolator%value_n(1:sz_value_n) =  &
    interpolator%interp1d_n%bcoef(1:sz_value_n)
...
!> @param[out] interpolator the type sll_arbitrary_degree_spline_interpolator_2d
subroutine set_coefficients_1d_ad2d( interpolator, coeffs_1d )

sll_interpolator_2d,      intent(inout)  :: interpolator
sll_real64, dimension(:), intent(in)     :: coeffs_1d
...
case(0) ! periodic-periodic
  ...
  do i= 1,sp_deg1 + 1
    do j=1,sp_deg2 + 1
      interpolator%bcoef(num_cells1+i,num_cells2+j) = interpolator%bcoef(i,j)
    end do
  end do
...
case (9) ! 2. dirichlet-left, dirichlet-right, periodic
  ...
  do i = 1 ,nb_spline_eta1
    do j = 1,nb_spline_eta2
      interpolator%bcoef(i+1,j) = coeffs_1d(i+nb_spline_eta1*(j-1))
    end do
  end do
...
case(576)!3. periodic, dirichlet-bottom, dirichlet-top
  ...
  do i = 1 , nb_spline_eta1
    do j = 1,nb_spline_eta2
      interpolator%bcoef(i ,j+1) = coeffs_1d(i+nb_spline_eta1 *(j-1) )
    end do
  end do
...
case(585) ! 4. dirichlet in all sides
  ...
  do i = 1,nb_spline_eta1
    do j = 1,nb_spline_eta2
       interpolator%bcoef(i+1,j+1) = coeffs_1d( i + nb_spline_eta1 *(j-1))
    end do
  end do
  ! ------------------------------------------------------------
...
case(2340) ! Hermite in al sides
  ...
  do i = 1,nb_spline_eta1
    do j = 1,nb_spline_eta2
      interpolator%bcoef(i,j) = coeffs_1d(i+nb_spline_eta1 *(j-1))
    end do
  end do
  ! ------------------------------------------------------------
end select

end subroutine !set_coefficients_1d_ad2d
...
!> @param[in]  size_knots2 the size of knots in the direction eta2
!> @param[out] interpolator the type sll_arbitrary_degree_spline_interpolator_2d
subroutine set_coefficients_2d_ad2d( interpolator,  &
                                     coeffs_2d,     &
                                     coeff2d_size1, &
                                     coeff2d_size2, &
                                     knots1,        &
...
sll_interpolator_2d,        intent(inout) :: interpolator
sll_real64, dimension(:,:), intent(in)    :: coeffs_2d
sll_int32,                  intent(in)    :: coeff2d_size1
sll_int32,                  intent(in)    :: coeff2d_size2
sll_real64, dimension(:),   intent(in)    :: knots1
sll_real64, dimension(:),   intent(in)    :: knots2
sll_int32,                  intent(in)    :: size_knots1
sll_int32,                  intent(in)    :: size_knots2
...
!PN Knots positions are already set in module De Boor splines
!PN interpolator%t1 ( 1 : sp_deg1 + 1 )  = eta1_min
!PN interpolator%t1 ( coeff2d_size1 + 2: coeff2d_size1 + 2 + sp_deg1) = eta1_max

!PN do i = 1, coeff2d_size1 -sp_deg1
!PN   interpolator%t1 ( i + sp_deg1 + 1 ) = eta1_min + &
!PN     i * (eta1_max - eta1_min) / (coeff2d_size1-sp_deg1 + 1)
!PN end do

!PN interpolator%t2 ( 1 : sp_deg2 + 1 )  = eta2_min
!PN interpolator%t2 ( coeff2d_size2 + 2: coeff2d_size2 + 2 + sp_deg2) = eta2_max

!PN do i = 1, coeff2d_size2 -sp_deg2
!PN   interpolator%t2 ( i + sp_deg2 + 1 ) = eta2_min + &
!PN     i * (eta2_max - eta2_min) / (coeff2d_size2-sp_deg2 + 1)
!PN end do
...
sll_int32  :: sz_derivative_eta1
sll_int32  :: sz_derivative_eta2
sll_int32  :: sz1
sll_int32  :: sz2
sll_real64 :: period1
sll_real64 :: period2
...
case (585) ! 4. dirichlet in all sides
    ...
    case (650) !left: Neumann, right: Dirichlet, bottom: Neumann, Top: Dirichlet
       ...
       data_array_deriv_eta1(2,1:sz2)     = interpolator%slope_e(1:sz2)
       point_location_eta2_deriv(1) = 1
       point_location_eta2_deriv(2) = sz2
       data_array_deriv_eta2(1,1:sz1+sz_derivative_eta1)=0.0_f64
       data_array_deriv_eta2(2,1:sz1+sz_derivative_eta1)=interpolator%slope_n(1:sz1+sz_derivative_eta1)
  ...
  !     interpolator%bcoef(1:sz1,sz2) = interpolator%value_n(1:sz1)
    ...
    case(657) !left: Dirichlet, right: Neumann, bottom: Neumann, Top: Dirichlet
       ...
       point_location_eta1_deriv(1) = 1
       point_location_eta1_deriv(2) = sz1
       data_array_deriv_eta1(1,1:sz2)     = interpolator%slope_w(1:sz2)
 ...
 !      interpolator%bcoef(1:sz1,1)   = interpolator%value_s(1:sz1)
  !     interpolator%bcoef(1:sz1,sz2) = interpolator%value_n(1:sz1)
    ...
    case(801)  !left: Dirichlet, right: Hermite, bottom: Hermite, Top: Dirichlet
       ...
       point_location_eta2_deriv(1) = 1
       point_location_eta2_deriv(2) = sz2
       data_array_deriv_eta2(1,1:sz1+sz_derivative_eta1)=interpolator%slope_s(1:sz1+sz_derivative_eta1)
       data_array_deriv_eta2(2,1:sz1+sz_derivative_eta1)=interpolator%slope_n(1:sz1+sz_derivative_eta1)
       call spli2d_custom_derder(&
            sz1,&
            sz_derivative_eta1,&

...
module sll_module_advection_2d_CSL1D
...
implicit none
  ...
  type,extends(sll_advection_2d_base) :: CSL1D_2d_advector
  ...
  contains
     procedure, pass(adv) :: initialize => &
       initialize_CSL1D_2d_advector
    procedure, pass(adv) :: advect_2d => &
      CSL1D_advect_2d
  
  end type CSL1D_2d_advector
...
contains
  function new_CSL1D_2d_advector( &
    interp, &
    charac, &
    Npts1, &
    Npts2, &
    ...
    eta2_coords) &  
    result(adv)      
    type(CSL1D_2d_advector), pointer :: adv
    class(sll_interpolator_2d_base), pointer :: interp
    class(sll_characteristics_2d_base), pointer  :: charac
    ...
    call initialize_CSL1D_2d_advector(&
      adv, &
      interp, &
      charac, &
      Npts1, &
  ...
  end function  new_CSL1D_2d_advector


  subroutine initialize_CSL1D_2d_advector(&
    adv, &
    interp, &
    charac, &
    Npts1, &
    ...
    eta1_coords, &
    eta2_coords)    
    class(CSL1D_2d_advector), intent(inout) :: adv
    class(sll_interpolator_2d_base), pointer :: interp
    class(sll_characteristics_2d_base), pointer  :: charac
    sll_int32, intent(in) :: Npts1
    sll_int32, intent(in) :: Npts2
    ...
    print *,'#Warning, for the moment, it is just a copy of BSL'
  ...
  end subroutine initialize_CSL1D_2d_advector

  subroutine CSL1D_advect_2d(&
    adv, &
    A1, &
    A2, &
    dt, &
...
!      adv%Npts2 )
  ...
  end subroutine CSL1D_advect_2d





end module sll_module_advection_2d_CSL1D

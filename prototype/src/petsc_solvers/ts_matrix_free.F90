...
PetscScalar, public :: PETSC_SHIFT
TS,          public :: tscontext
Mat,         public :: Jmat
PetscReal,   public :: MFuser(6)
...
use petscdmda
  ...
  PetscReal            :: user(6)
  PetscErrorCode       :: ierr
  integer, parameter   :: user_a=1
  integer, parameter   :: user_k=3
  integer, parameter   :: user_s=5
  ...
  call VecGetArrayF90(Xdot,xxdot,ierr)
  call VecGetArrayF90(F,ff,ierr)

  call FormIFunctionLocal(mx,xs,xe,gxs,gxe,       &
       xx,xxdot,ff,user(user_a),user(user_k),user(user_s),ierr)

  call VecRestoreArrayF90(X,xx,ierr)
  call VecRestoreArrayF90(Xdot,xxdot,ierr)
...
use petscdmda
  ...
  ! Get access to vector data
  call VecGetArrayF90(Xloc,xx,ierr)
  call VecGetArrayF90(F,ff,ierr)

  call FormRHSFunctionLocal(mx,xs,xe,gxs,gxe,t,xx,ff,                  &
       user(user_a),user(user_k),user(user_s),ierr)

  call VecRestoreArrayF90(Xloc,xx,ierr)
  call VecRestoreArrayF90(F,ff,ierr)
...
use petscdmda
  ...
  PetscReal          :: user(6)
  PetscErrorCode     :: ierr
  integer, parameter :: user_a=0
  integer, parameter :: user_k=2
  integer, parameter :: user_s=4
...
use petscdmda
  ...
  call FormInitialSolutionLocal(mx,xs,xe,gxs,gxe,                       &
       xx,user(user_a),user(user_k),user(user_s),ierr)

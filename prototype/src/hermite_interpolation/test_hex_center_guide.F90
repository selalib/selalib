...
#include "sll_assert.h"
  ...
  sll_int32    :: num_cells, n_points, n_triangle, n_edge, n_min, n_min_center, n_min_edge, n_min2, k_min
  sll_real64   :: center_mesh_x1, center_mesh_x2, radius, r_min
  ...
  do num_cells = 60,60,40 
     ...
     if (k_min>0) then
        ...
        n_min_center = 6*k_min**k_min
        n_min_edge = 3*k_min*(3*k_min+1)
        n_min2 = 1+6*k_min*(2*k_min-1)
     else
        n_min = 0
        n_min_center = 0
        n_min_edge = 0
     ...
     call init_distr(rho_tn,rho_center_tn,rho_edge_tn,num_method,mesh, n_min, n_min_center, n_min_edge)
     ...
     if ( num_method == 15 ) then
        ...
        call hex_matrix_poisson( matrix_poisson2, mesh2,n_min2,k_min*2)
        ...
        call hex_second_terme_poisson( second_term2, mesh2, rho2, n_min2, k_min*2)
        ...
        call compute_hex_fields(mesh2,uxn2,uyn2,dxuxn2,dyuxn2,dxuyn2,dyuyn2,phi2,n_min,k_min)
     ...
     if ( num_method /= 15 ) then
        call hex_diagnostics(rho_tn,t,mesh,uxn,uyn,nloops, num_method,n_min)
     else
        call hex_diagnostics(rho2,t,mesh2,uxn2,uyn2,nloops, num_method,n_min)
     endif
     ...
     do while (t .lt. tmax)
        ...
        call  der_finite_difference( rho_tn, p, step, mesh, deriv, n_min, k_min  )
        ...
        do i= n_min + 1, n_points 
           ...
           !if (t < 2*dt) then
              ...
              ! if ( abs(h1) <  r_min .or. abs(h2) <  r_min &
              !      .or. abs(xx) < (r_min)*sqrt(3._f64)*0.5_f64) inside = .false.

              if ( inside ) then
                 call hermite_interpolation(i, xx, yy, rho_tn, rho_center_tn,&
                      rho_edge_tn, rho_tn1, mesh, deriv, aire, n_min, k_min ,& 
                      num_method)
              else 
                 rho_tn1(i) = 0._f64 ! null dirichlet boundary condition for both inner and outer boundary
              endif
           ...
           !            rho_edge_tn_1, rho_tn1, mesh, deriv, aire, n_min, k_min ,& 
           !            num_method)
           !    else 
           ...
           !         .or. abs(xx) > (radius-mesh%delta)*sqrt(3._f64)*0.5_f64) inside = .false.
           !    ! inner boundary condition
           !    if ( abs(h1) <  r_min+mesh%delta .or. abs(h2) <  r_min+mesh%delta &
           !         .or. abs(xx) < (r_min+mesh%delta)*sqrt(3._f64)*0.5_f64) inside = .false.
        ...
        if ( num_method == 10 ) then 
           ...
           do i = n_min_center+1,mesh%num_triangles
              ...
              if ( inside ) then
                 call hermite_interpolation(i, x, y, rho_tn, rho_center_tn,&
                      rho_edge_tn, rho_center_tn1, mesh, deriv, aire, n_min, k_min ,& 
                      num_method)
              else 
        ...
        if ( num_method == 15 ) then 
           ...
           do i= n_min_edge + 1, n_edge
              ...
              if ( inside ) then
                 call hermite_interpolation(i,xx,yy,rho_tn,rho_center_tn&
                      ,rho_edge_tn, rho_edge_tn1,mesh,deriv,aire, n_min, k_min ,&
                      num_method)
              else 
        ...
        if ( num_method == 15 ) then
           call assemble(rho_tn,rho_edge_tn,rho2,mesh,mesh2)
           call hex_second_terme_poisson( second_term2, mesh2, rho2, n_min2, 2*k_min)
           call solvlub_bande(l2,u2,phi2_interm,second_term2,n_points2,width_band1_2,width_band2_2)
...
contains
  ...
  subroutine init_distr(f_tn,center_values_tn,edge_values_tn,num_method,mesh,n_min, n_min_center, n_min_edge)
    type(sll_hex_mesh_2d), pointer :: mesh
    sll_real64,dimension(:)        :: f_tn, center_values_tn, edge_values_tn
    sll_int32  :: num_method, n_min, n_min_center, n_min_edge
    sll_real64 :: x, y, epsilon = 1e-1
    sll_real64 :: rho
    sll_real64 :: r
    sll_int32  :: i

    if ( n_min>0 ) f_tn(1:n_min) = 0._f64
    ...
    if ( num_method == 10 ) then 
        ...
        if ( n_min_center>0 ) center_values_tn(1:n_min_center) = 0._f64
    ...
    if ( num_method == 15 ) then 
        ...
        if ( n_min_edge>0 ) edge_values_tn(1:n_min_edge) = 0._f64

       do i = n_min_edge+1,mesh%num_edges
          x = mesh%edge_center_cartesian_coord(1,i)
          y = mesh%edge_center_cartesian_coord(2,i)
          r = sqrt( x**2 + y**2 )
  ...
  subroutine hex_diagnostics(rho,t,mesh,uxn,uyn,nloop, num_method, n_min)
    type(sll_hex_mesh_2d), pointer :: mesh
    sll_real64,dimension(:) :: rho,uxn,uyn
    sll_real64,intent(in)   :: t
    sll_int32 ,intent(in)   :: nloop,num_method, n_min

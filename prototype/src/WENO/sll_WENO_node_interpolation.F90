...
contains  ! ****************************************************************
  ...
  function new_WENO_1D(num_points, xmin, xmax)
    ...
    sll_int32                   :: ierr  ! allows for an error code return in 
                                         ! allocating memory
    sll_int32                   :: i_temp
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  function interpolate_WENO_1D(this, num_points, data, coordinates) result(res)
    type(sll_WENO_1D), intent(in)  :: this   
...
!!!ES modified Jingmei's version so that data are at grid points 
!!!! and not at cell centers so that points 1 and num_points are same point
...
!!! interpolation based on f_temp
    do ic_temp = 1, num_points
       ...
       xi_temp = (coordinates(ic_temp)-(this%xmin+(i_temp-1.0_f64) &
            *this%delta))*this%rdelta
       ...
       g1 = f_temp(i_temp) &
            ...
            +2.0_f64*f_temp(i_temp-2)-.5_f64*f_temp(i_temp-3)) &
            *xi_temp**2.0_f64 &
            + (f_temp(i_temp)/6.0_f64-f_temp(i_temp-1)/2.0_f64 &
            +f_temp(i_temp-2)/2.0_f64-f_temp(i_temp-3)/6.0_f64) &
            *xi_temp**3.0_f64
       g2 = f_temp(i_temp) &
              ...
              +f_temp(i_temp-1)/2.0_f64-f_temp(i_temp-2)/6.0_f64) &
              *xi_temp**3.0_f64
       g3 = f_temp(i_temp) &
            ...
            +f_temp(i_temp)/2.0_f64-f_temp(i_temp-1)/6.0_f64)*xi_temp**3.0_f64
  ...
  subroutine delete_WENO_1D( fWENO )
    ...
    ! for instance
    SLL_ASSERT( associated(fWENO%data) )

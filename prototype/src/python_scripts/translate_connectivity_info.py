...
def main ():
    ...
    for i in range(3, int(num_patches)+3):
        with open(args[i],'r') as readfile, open(patch_li_syms[i-3],'w') \
            as writefile:
            print("converting data from: " + args[i] + " to " + 
                  patch_li_syms[i-3])
    ...
    for i in range(int(num_patches)):
        with open(args[int(num_patches)+3+i],'r') as readfile, \
                open(patch_loc_glob_syms[i],'w') as writefile:
            print("converting data from: " + args[int(num_patches)+3+i] +
                  " to " + patch_loc_glob_syms[i])
            ...
            for line in linelist:
                ...
                if rows == 0:
                    ...
                    if len(linetemp) != cols:
                        print("There is something suspicious in file: ")
                        print(args[int(num_patches)+3+i])
                        print("regarding the size of the data. The number ")
                        print("of columns changes from one row to the next.")
                        print(cols)
                rows = rows + 1

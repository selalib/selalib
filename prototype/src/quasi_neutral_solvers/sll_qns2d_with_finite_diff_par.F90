...
module sll_qns2d_with_finite_diff_par
...
#include "sll_remap.h"
  ...
  type qns2d_with_finite_diff_plan_par
...
contains
  ...
  function new_qns2d_with_finite_diff_plan_par(BC,rmin,rmax,NP_r, NP_theta) &
                                                                 result (plan)
    ...
    ! NP_r and NP_theta are the numbers of points in directions r and theta 
    ! respectively
    sll_int32                                      :: NP_r_loc, NP_theta_loc
    ! NP_r_loc and NP_theta_loc are the numbers of points locally (in 
    ! the processor) in directions r and theta respectively
    sll_int32                                      :: ierr
    sll_int64                                      :: colsz
  ...
  end function new_qns2d_with_finite_diff_plan_par


 subroutine solve_qns2d_with_finite_diff_par(plan, rho, c, Te, f, g, Zi, phi)
    ...
    do j=1,NP_theta_loc
       ...
       ind = global(2)
       if (plan%BC=='neumann') then
          call neumann_matrix_resh_par(plan, ind-1, real(plan%c_remap( &
              :,1,1), f64), real(plan%Te_remap(:,1,1), f64), Zi, a_resh)
       else ! 'dirichlet'
          call dirichlet_matrix_resh_par(plan, ind-1, real(plan%c_remap( &
                :,1,1), f64), real(plan%Te_remap(:,1,1), f64), Zi, a_resh)
       endif 
  ...
  end subroutine solve_qns2d_with_finite_diff_par


  subroutine delete_qns2d_with_finite_diff_plan_par(plan)
  ...
  end subroutine delete_qns2d_with_finite_diff_plan_par
...
end module sll_qns2d_with_finite_diff_par

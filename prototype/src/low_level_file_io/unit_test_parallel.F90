...
contains
  ...
  CALL h5fcreate_f("layout1.h5",H5F_ACC_TRUNC_F,file_id,error,access_prp = plist_id)
  ...
  CALL h5dcreate_f(file_id, "local_array1", H5T_NATIVE_DOUBLE, filespace, &
  ...
  offset = 0
  if (istart > 0) offset(1) = istart
  if (jstart > 0) offset(2) = jstart
  if (kstart > 0) offset(3) = kstart
  ...
  call file_layout1%write_array(dimsf,offset,local_array1,'array1',error)

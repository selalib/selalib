...
#include "sll_memory.h"
  ...
  sll_int    :: i,j,N_phi,err,N_x1,N_x2,i1,i2,N
  LOGICAL :: ex
  ...
  sll_real64 :: phi_val,delta_x1_phi,xx,dt,alpha
  sll_int :: ii
  ...
  do i1=1,N_x1+1
    do i2=1,N_x2+1
      ...
      f(i1,i2) = mu/(sqrt(2._f64*sll_pi))*(2._f64-2._f64*xi)/(3._f64-2._f64*xi)*(1._f64+H/(1._f64-xi))*exp(-H)
    enddo
  enddo
  ...
  do i2=1,N_x2+1
    ...
    f(1:N_x2+1,i2) = buf_1d(1:N_x1+1)
  enddo
  
  !compute_rho
  do i1=1,N_x1+1
    buf_1d(1:N_x2+1) = f(i1,1:N_x2+1)
    !rho(i1) = compute_non_unif_integral(node_positions_x2(1:N_x2+1),buf_1d(1:N_x2+1))-1._f64
  enddo
...
function compute_non_unif_integral(x_points,f_points,N_points)
  use numeric_constants
  implicit none
  sll_real64 :: compute_non_unif_integral
  sll_real64,dimension(:),pointer :: x_points,f_points
...
subroutine poisson1dpertrap(E,L,N)
  ...
  implicit none
  sll_int,intent(in)::N
  sll_real64,dimension(N),intent(inout)::E

...
#include "sll_assert.h"
  ...
  type sll_plan_poisson_polar
     ...
     !for quasi neutral solver
     sll_real64, dimension(:), pointer ::dlog_density,inv_Te
...
!===================
  ...
  !>phi : unknown field, size (nr+1)*(ntheta+1), output
  !>initialization must be done outside the solver
  subroutine poisson_solve_polar_dk(plan,f,phi,ierr)
  ...
  end subroutine poisson_solve_polar_dk

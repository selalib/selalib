...
SLL_ALLOCATE(es%local_spline_indices(dim1,dim2),ierr)
SLL_ALLOCATE(es%local_to_global_spline_indices(dim1,dim2),ierr)
SLL_ALLOCATE(es%local_to_global_spline_indices_source(dim1,dim2),ierr)
SLL_ALLOCATE(es%local_to_global_spline_indices_source_bis(dim1,dim2),ierr)
...
case default
  SLL_WARNING('new general solver have not type of gauss points in eta1')
end select
...
case default
  SLL_WARNING('new general solver have not type of gauss points in eta2')
end select
...
call initialize_knots_source( spline_degree1,  &
                              num_cells1,      &
                              eta1_min,        &
                              eta1_max,        &
                              es%knots1_rho)

call initialize_knots_source( spline_degree2,  &
                              num_cells2,      &
                              eta2_min,        &
                              eta2_max,        &
                              es%knots2_rho)
...
dim1 = num_cells1*(spline_degree1+2)
dim2 = num_cells2*(spline_degree2+2)
SLL_CLEAR_ALLOCATE(es%values_splines1(1:dim1,1:spline_degree1+1),ierr)
SLL_CLEAR_ALLOCATE(es%values_splines2(1:dim2,1:spline_degree2+1),ierr)
SLL_CLEAR_ALLOCATE(es%values_jacobian(1:dim1,1:dim2),ierr)
SLL_CLEAR_ALLOCATE(es%values_splines_gauss1(1:dim1,1:spline_degree1+1),ierr)
SLL_CLEAR_ALLOCATE(es%values_splines_gauss2(1:dim2,1:spline_degree2+1),ierr)

SLL_ALLOCATE(es%tab_index_coeff1(dim1),ierr)
SLL_ALLOCATE(es%tab_index_coeff2(dim2),ierr)
...
num_quads = es%num_cells1*es%num_cells2

total_num_splines_loc = es%total_num_splines_loc

SLL_ALLOCATE(Source_loc(num_quads,total_num_splines_loc,total_num_splines_loc),ierr)
...
do j=1,es%num_cells2
  do i=1,es%num_cells1
    ...
    call local_to_global_matrices( es,           &
                                   cell_index,   &
                                   i,            &
                                   j,            &
                                   Masse_loc,    &
...
if (es%perper) then
  ...
  do i = 1, es%total_num_splines1*es%total_num_splines2    
    ...
    call sll_add_to_csr_matrix( es%sll_csr_mat_with_constraint,                &
                                es%masse(i),                                   &
...
class is (sll_scalar_field_2d_discrete)
  ...
  class is (sll_arbitrary_degree_spline_interpolator_2d)
    ...
    call sll_mult_csr_matrix_vector( es%sll_csr_mat_source, &
                                     es%rho_coeff_1d,       &
                                     es%rho_vec)
  ...
  class default
    ...
    do j=1,es%num_cells2
     ...
     do i=1,es%num_cells1
      ...
      do jg=1,num_pts_g2
       gpt2  = eta2 + 0.5_f64*es%delta_eta2 * (es%gauss_pts2(1,jg)+1)
       wgpt2 = 0.5*es%delta_eta2*es%gauss_pts2(2,jg) 
       ig2 = jg + (j-1)*num_pts_g2
       do ig=1,num_pts_g1
        gpt1  = eta1 + 0.5_f64*es%delta_eta1 * (es%gauss_pts1(1,ig)+1)
        wgpt1 = 0.5*es%delta_eta1*es%gauss_pts1(2,ig)
...
class is (sll_scalar_field_2d_analytic)
  ...
  do j=1,es%num_cells2
    ...
    do i=1,es%num_cells1
      eta1  = es%eta1_min + (i-1)*es%delta_eta1
      do jg=1,num_pts_g2
        gpt2  = eta2 + 0.5*es%delta_eta2 * (es%gauss_pts2(1,jg)+1)
        wgpt2 = 0.5*es%delta_eta2*es%gauss_pts2(2,jg)
        ig2 = jg+(j-1)*num_pts_g2
        do ig=1,num_pts_g1
          gpt1  = eta1 + 0.5*es%delta_eta1 * (es%gauss_pts1(1,ig)+1)
...
subroutine build_local_matrices( obj,                      &
                                 cell_index,               &
                                 cell_i,                   &
                                 cell_j,                   &
                                 a11_field_mat,            &
...
! The supposition is that all fields use the same mesh
...
do j=1,num_pts_g2
  ...
  call interv( obj%knots2_rho,                          &
               obj%num_cells2 + obj%spline_degree2+ 2,  &
               gpt2,                                    &
               left_y,                                  &
               mflag_y )

  call bsplvd( obj%knots2_rho,                          &
               obj%spline_degree2+1,                    &
               gpt2,                                    &
               left_y,                                  &
               work2,                                   &
  ...
  obj%values_splines2(cell_j + obj%num_cells2*(j-1),:) = dbiatx2(:,1)
  ...
  do i=1,num_pts_g1
    ...
    call interv( obj%knots1_rho,                         &
                 obj%num_cells1 + obj%spline_degree1+ 2, &
                 gpt1,                                   &
                 left_x,                                 &
                 mflag_x )
   
    call bsplvd( obj%knots1_rho,         &
                 obj%spline_degree1+1,   &
                 gpt1,                   &
                 left_x,                 &
                 work1,                  &
    ...
    obj%values_splines1(cell_i + obj%num_cells1*(i-1),:) = dbiatx1(:,1)
    ...
    obj%values_jacobian(cell_i + obj%num_cells1*(i-1),cell_j &
                               + obj%num_cells2*(j-1)) = val_jac
    ...
    do ii = 0,obj%spline_degree1
       do jj = 0,obj%spline_degree2
         ...
         Masse_loc(index1) = Masse_loc(index1) + &
           val_jac*wgpt1*wgpt2*(dbiatx1(ii+1,1)*dbiatx2(jj+1,1))

         Stiff_loc(index1) = Stiff_loc(index1) + &
           val_jac*wgpt1*wgpt2*                  &
...
subroutine build_local_matrices_rho( obj, cell_i, cell_j)
...
do j=1,num_pts_g2
  ...
  do i=1,num_pts_g1
    ...
    val_jac = obj%values_jacobian(cell_i+obj%num_cells1*(i-1),cell_j+obj%num_cells2*(j-1))
   ...
   ! each direction.
    do ii = 0,obj%spline_degree1
      do jj = 0,obj%spline_degree2
        ...
        spline1 = obj%values_splines1(cell_i + obj%num_cells1*(i-1),ii+1)
...
subroutine local_to_global_matrices( es,               &
                                     cell_index,       &
                                     cell_i,           &
                                     cell_j,           &
                                     Masse_loc,        &
...
do mm = 0,es%spline_degree2
  ...
  do i = 0,es%spline_degree1
    ...
    A =  es%local_to_global_spline_indices(b, cell_index)
    ...
    es%local_to_global_spline_indices_source(b,cell_index)= &
      index_coef1 + (index_coef2-1)*(es%num_cells1+1)
    ...
    do nn = 0,es%spline_degree2
      ...
      do j = 0,es%spline_degree1
        ...
        Aprime = es%local_to_global_spline_indices(bprime,cell_index)

        elt_mat_global = M_c_loc(b, bprime)     - &
                         K_a11_loc(b, bprime)   - &
                         K_a12_loc(b, bprime)   - &
                         K_a21_loc(b, bprime)   - &
                         K_a22_loc(b, bprime)   - &
        ...
        if (A > 0 .and. Aprime > 0) then
          ...
          call sll_add_to_csr_matrix( es%sll_csr_mat, &
                                      elt_mat_global, &
                                      A,              &
                                      Aprime          )   
        end if
...
call sll_solve_csr_matrix(es%sll_csr_mat, es%tmp_rho_vec, es%tmp_phi_vec)
...
type(general_coordinate_elliptic_solver),intent(inout) :: es
sll_real64, dimension(:,:,:), pointer,   intent(in)    :: Source_loc
  ...
  do cell_i= 1, es%num_cells1
    ...
    do ideg2 = 0, es%spline_degree2
      ...
      do ideg1 = 0, es%spline_degree1
        ...
        A = es%local_to_global_spline_indices_source_bis(b, cell_index)
        ...
        do jdeg2 = 0, es%spline_degree2
           ...
           do jdeg1 = 0, es%spline_degree1
             ...
             Aprime = es%local_to_global_spline_indices_source(bprime,cell_index)
                  
             elt_mat_global = Source_loc(cell_index,bprime,b)

             if ( A>0 .and. Aprime>0 ) then

                call sll_add_to_csr_matrix( es%sll_csr_mat_source, &
                                            elt_mat_global,        &
                                            A,                     &
                                            Aprime                 )
             end if

...
subroutine schex ( r, ldr, p, k, l, z, ldz, nz, c, s, job )
  ...
  !      (   -S(I)       C(I) )
  !
  !    where C(I) is real(8), the rows these rotations operate on
  !    are described below.
  !
        ...
        do j = l + 1, p
          ...
          if ( j >= pl .and. j <= pu ) then
            ...
            if ( qraux(j) /= 0.0E+00 ) then
              ...
              tt = 1.0_8 - ( abs ( a(l,j) ) / qraux(j) )**2
              tt = max ( tt, 0.0_8 )
              t = tt
              tt = 1.0_8 + 0.05_8 * tt * ( qraux(j) / work(j) )**2
...
!
  ...
  if ( abs ( c ) > 0.0_8 .and. abs ( c ) <= s ) then
    z = 1.0_8 / c
  else
    z = s
  end if
  ...
  if ( wantu ) then
    ...
    do ll = 1, nct
      ...
      if ( s(l) /= 0.0_8 ) then
        ...
        call sscal ( n-l+1, -1.0_8, u(l,l), 1 )
        u(l,l) = 1.0_8 + u(l,l)
        u(1:l-1,l) = 0.0_8

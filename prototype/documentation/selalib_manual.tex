...
Available functions:
\begin{verbatim}
     new_spline_1D( data, num_cells, xmin, xmax, bc_type )
\end{verbatim}
\verb+new_spline_1D()+ is responsible for allocating all the necessary storage for the spline object and its initialization. Hence the full creation of a spline is a two-step process. The first one is the declaration of the spline pointer, the second is its assignment through a call to the \verb+new_spline_1D()+ function. For details, see the usage section below. In this function:
\begin{description}
\item[data:] 
The double precision floating point data array to be fitted.
\item[num\_cells:] 
The data array represents the values of a function at the nodes of a uniform 1D grid. \verb+num_cells+ represents the number of cells in this uniform grid. For periodic boundary conditions one may choose that the number of cells equals the number of data points in the array. In other words, for a data array with points indexed 1:NP, we can think that these data represent a grid with NP cells, and where the following point (NP+1) would be the same as point 1. In case of hermitian boundary conditions, a data array indexed 1:NP would correspond to a grid of size NP-1 cells, as both endpoints are included.
\item[xmin:]
...
\item[xmax:]
Similarly to \verb+xmin+, \verb+xmax+ represents the maximum extent of the grid. Notice, however, that this is influenced by how one thinks of the data in the grid; for example, consider the case of periodic data defined over a grid of NC cells and indexed 1:NC (that is, the point NC+1 is the same as the point 1 and is not taken into account). In this case, $xmax = x_{NC+1}$. In such way that $xmin - xmax$ is really the period of the function. In the case of data indexed 1:NC+1, and hermitian boundary conditions, then $xmax = x_{NC+1}$, the location of the last point. 
\item[bc\_type:]
...
\begin{verbatim}
     interpolate_value( x, spline )
\end{verbatim}
\verb+interpolate_value()+ returns the value of \emph{f(x)} where \emph{x} is a floating-point value between \verb+xmin+ and \verb+xmax+, and \emph{f} is a continuous function built with cubic B-splines and user-defined boundary conditions. Essentially, the spline object and the interpolate function create the illusion of having available a continuous function when originally, only discrete data were available.
Available subroutines:
\begin{verbatim}
     compute_spline_1D( data, num_cells, bc_type, spline )
\end{verbatim}
\verb+compute_spline_1D()+ is responsible for actually computing the spline information (i.e.: the spline coefficients) when one does not wish to allocate a new object. This can be thought of a way to \emph{update} the spline coefficients of a given spline object. In this subroutine:
\begin{description}
\item[data:] 
The double precision floating point data array to be fitted.
\item[num\_cells:] 
...
\begin{verbatim}
     compute_spline_1D_periodic( data, num_cells, spline )
     compute_spline_1D_hermite ( data, num_cells, spline )
\end{verbatim}
These are subroutines are meant as an alternative interface to the services of \verb+compute_spline_1D()+, in which the required boundary condition is explicit in the name of the function. These functions are slightly faster in that the programmer is choosing the boundary condition, and thus a branch can be avoided in the former function.

\begin{verbatim}
     delete_spline_1D( spline )
\end{verbatim}
\verb+delete_spline_1D()+ is the spline destructor. Its use is important to avoid memory leaks. As usual, dummy arguments that refer to fundamental types (like spline) are pointers to such types.
...
17
18      accumulator1 = 0.0_f64
19      accumulator2 = 0.0_f64
20    
21      SLL_ALLOCATE(data(NC+1), err)
22 
23      print *, 'initialize data array'
24      do i=1,NC+1
25        data(i) = sin((i-1)*sll_pi/real(NC,f64))
26      end do
27
28      sp1 =>  new_spline_1D( data(1:NC), NC, 0.0_f64, sll_pi, PERIODIC_SPLINE )
29      sp2 =>  new_spline_1D( data(1:(NC+1)), NC, 0.0_f64, sll_pi, HERMITE_SPLINE )
30  
31      print *, 'cumulative errors at nodes: '
32      do i=1, NC+1
33         accumulator1 = accumulator1 + abs(data(i) - &
34              interpolate_value(real(i-1,f64)*sll_pi/real(NC,f64), sp1))
35      end do
36
37      print *, 'hermite case: '
38      do i=1, NC+1
39         accumulator2 = accumulator2 + abs(data(i) - &
40              interpolate_value(real(i-1,f64)*sll_pi/real(NC,f64), sp2))
41      end do
42      print *, 'Periodic case: '
43      print *, 'average error at the nodes = '
44      print *, accumulator1/real(NC,f64)
45      call delete_spline_1D(sp1)
46      if( accumulator1/real(NC,f64) < 1.0e-15 ) then 
47         print *, 'PASSED TEST'
48      else
49         print *, 'FAILED TEST'
50      end if
51      print *, '**************************** '
52      print *, 'Hermite case: '
53      print *, 'average error at the nodes = '
54      print *, accumulator2/real(NC,f64)
55      call delete_spline_1D(sp2)
56      if( accumulator2/real(NC,f64) < 1.0e-15 ) then 
57         print *, 'PASSED TEST'
58      else
59         print *, 'FAILED TEST'
60      end if
61    end program spline_tester
\end{verbatim}
...
\item[Lines 28 - 29:]
Allocation and initialization of the splines. Note the \verb+=>+ pointer assignment syntax.
...
In this example we do not show the use of the \verb+compute_spline_1D()+ functions. These are used though, albeit indirectly. For example, the initialization of a spline object uses the  \verb+compute_spline_1D()+ internally. We could have also at some point in the program made a direct call such as

\begin{verbatim}
     call compute_spline_1D( other_data, NC, PERIODIC_SPLINE, sp1 )
\end{verbatim}
and updated the contents of \verb+sp1+.

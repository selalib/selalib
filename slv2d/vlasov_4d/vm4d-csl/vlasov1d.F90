...
contains
  ...
  subroutine advection1d_x(this,f,dt,Jx1,meth)
    !-----------------------------------------------
    ...
    sll_int32 :: i, ix, jx, iv, jv, c ! indices de boucle

    ! verifier que la transposition est a jours
    if (this%transpose) stop 'advection_x: on travaille sur f et pas ft'

    locJx1=0._wp;Jx1(:,:)=0._wp;!locflux=0._8

    do jv=this%jstartv,this%jendv
       do iv=1,this%geomv%nx
          vx = this%geomv%x0+real(iv-1,8)*this%geomv%dx
          depx = vx*dt
          do jx = 1,this%geomx%ny
             f1dx(:)=f(:,jx,iv,jv)
             !compute f_i = fn_i-(flux_{i+1/2}-flux_{i-1/2})
             call interpole(this,f1dx,depx,flux,meth)
             f(:,jx,iv,jv)=f1dx(:)
             !compute local current J=sum_{jv, iv} flux/dt dv^2
             locJx1(:,jx)=locJx1(:,jx)+vx*flux(:)*this%geomv%dx*this%geomv%dy 
          enddo    
       end do
    end do
...
!    Jx1(1:this%geomx%nx,1:this%geomx%ny)=locJx1(1:this%geomx%nx,1:this%geomx%ny)

  end subroutine advection1d_x

 subroutine advection1d_y(this,f,dt,Jy1,meth)
    !-----------------------------------------------
    ...
    do jv=this%jstartv,this%jendv
       vy = this%geomv%y0+real(jv-1,8)*this%geomv%dy
       depy = vy*dt
       do iv=1,this%geomv%nx
          do ix=1,this%geomx%nx
             f1dy(:)=f(ix,:,iv,jv)
             call interpole(this,f1dy,depy,flux,meth)
             f(ix,:,iv,jv)=f1dy(:)
             locJy1(ix,:)=locJy1(ix,:)+vy*flux(:)*this%geomv%dx*this%geomv%dy 
          enddo
       end do
    end do

...
subroutine densite_courant(this, jx, jy)
   ...
   sll_real64:: dx,dy,massx,massy
   sll_int32 :: i,j,iv,jv,c,nx,ny
   sll_int32 :: comm
   ...
   dx=this%geomx%dx;dy=this%geomx%dy
   nx=this%geomx%nx;ny=this%geomx%ny   

   massx=sum(jx(1:nx-1,1:ny-1))
   massy=sum(jy(1:nx-1,1:ny-1))
   jx(1:nx-1,1:ny-1)=jx(1:nx-1,1:ny-1)-massx/(real(nx-1,8)*real(ny-1,8))
   jy(1:nx-1,1:ny-1)=jy(1:nx-1,1:ny-1)-massy/(real(nx-1,8)*real(ny-1,8))
   jx(nx,:)=jx(1,:);jy(nx,:)=jy(1,:)
   jx(:,ny)=jx(:,1);jy(:,ny)=jy(:,1)
...
subroutine thdiag_tab(this,f,nrjtab,t,jstartv)

   type(vlasov2d),intent(inout) :: this
   sll_int32, intent(in) :: jstartv
   sll_real64, dimension(:,:,:,jstartv:),intent(in) :: f
   sll_int32 :: error
   sll_real64, intent(in) :: t,nrjtab(4)   ! current time
   ! variables locales
   sll_int32 :: i,iv, j,jv
   sll_real64 :: x, vx, y, vy
   sll_real64,dimension(7) :: diagloc
   sll_real64,dimension(11) :: auxloc
   sll_real64,dimension(16) :: aux
   sll_real64,dimension(0:9) :: diag
   sll_int32 :: my_num, num_threads
   sll_int32 :: comm

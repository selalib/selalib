...
#-----------------------------------------------------
# Fourier transform (not discrete) between 2 times
# and between two frequencies
#-----------------------------------------------------
def TFourier1D(F0, x, k_max, k_min, Nk):
    """ Fourier transform (not discrete) """

    dk  = (k_max - k_min) / Nk
    kx  = np.arange(k_min, k_max+dk, dk)
    s   = F0 * np.exp(- 1j * outer( kx, x ) )
    dt  = x[1] - x[0]
    TF  = s.sum(axis = 1) / dt

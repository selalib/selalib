...
class VP4Dresu():    
    ...
    #---------------------------------------
    # VP4Dresu initialisation
    #---------------------------------------
    def __init__(self,VP4Ddir=''):
    ...
    def read_filename_list(self,fname_list,nfile,ibeg,iend):
        ...
        exclude_vars = ['xgrid_2d','ygrid_2d']

        #--> Load the first file
        H = GYSut.loadHDF5(fname_list[ibeg])
        for iname in H.keys:
            if (iname not in exclude_vars):
                tupple_tmp = (nfile,)
...
#---------------------------------------------------------
# Ask which directory contains VP4D results to be 
#  analysed
#---------------------------------------------------------
def Ask_directory():

    """ Ask which directory contains VP4D results
    to be analysed"""
    ...
    nb_VP4D = np.size(glob.glob(directory+'/diags_*.h5'))
    if ( (nb_VP4D==0) ):
        print '\033[0;31m',\
            ' ==> There are NO VP4D results HDF5 files in :',\
...
def Ask_resultFiles(diagnum_list):
    ...
    print '';
    print ' *** VP4D hybrid diagnostics : choice of files *** '

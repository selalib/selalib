...
#include "sll_memory.h"
  ...
  use simulation_VP4D_module
  use sll_m_timer

  implicit none

  character(len=256)        :: input_filename
  type(sll_simulation_VP4D) :: simu_VP4D
  ...
  call simu_VP4D%init_from_file(trim(input_filename))

  !*** Running ot the 4D drift-kinetic simulation ***
  if ( my_rank==0 )then
    print *, '# *** Run VP4D: START '
  end if
  call simu_VP4D%run( )

...
!
   ...
   if (bctype_x == SLL_P_PERIODIC) then

      ibdry       = 0
      periods(1)  = .true.

   else if (bctype_x == SLL_P_NEUMANN) then

      ibdry       = 1
      periods(1)  = .false.

   else if (bctype_x == SLL_P_DIRICHLET) then
   ...
   if (bctype_y == SLL_P_PERIODIC) then

      jbdry       = 0
      periods(2)  = .true.

   else if (bctype_y == SLL_P_NEUMANN) then

      jbdry       = 1
      periods(2)  = .false.

   else if (bctype_y == SLL_P_DIRICHLET) then
...
subroutine write_topology( this )
   ...
   if ( prank == 0) then
      ...
      call sll_s_ascii_file_create("mpi_topology.mtv", file_id, error)

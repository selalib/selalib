...
#include "sll_working_precision.h"
  ...
  character(len = 256) :: diag_nc_filename
  character(len = 4)   :: filenum
  character(len = 4)   :: str_num_run
  character(len = 4)   :: str_num_cells

  sll_int32 :: num_cells
  sll_int32 :: cells_min, cells_max
  sll_int32 :: cells_step
  ...
  diag_nc_filename = "diag_nc.dat"
  call sll_s_ascii_file_create(diag_nc_filename, diag_nc_id, ierr)

  do num_cells = cells_min,cells_max,cells_step
     ...
     case ("SLL_LINEAR_ADV")
        ...
        print*, ""
        print*," *********************************************** "
        print*,"         Linear advection Simulation"
        print*,"       on a Hexagonal mesh of cells=", num_cells
        print*,"        using boxsplines of deg =", spline_degree
        print*," *********************************************** "
     ...
     case default
        SLL_ERROR("advection_2d_hexagonal", "bad value of model_name")
     end select
     ...
     call sll_s_int2string(num_cells, str_num_cells)
     if(use_num)then
        if (model_name .eq. "SLL_LINEAR_ADV") then
           base_name = &
                "linadv_"//trim(str_num_run)//"_nc"//trim(str_num_cells)//"_"
        elseif (model_name .eq. "SLL_CIRCULAR_ADV") then
           base_name = &
                "ciradv_"//trim(str_num_run)//"_nc"//trim(str_num_cells)//"_"
        else
           base_name = trim(str_num_run)//"_nc"//trim(str_num_cells)//"_"
        end if
        dist_name = "dist_"//trim(base_name)
        dist_error_name = "dist_error_"//trim(base_name)//"_"
        dist_exact_name = "dist_exact_"//trim(base_name)//"_"
        thdiag_1d_filename = "thdiag_1d_"//trim(base_name)//".dat"
     else
        if (model_name .eq. "SLL_LINEAR_ADV") then
           base_name = "linadv_nc"//trim(str_num_cells)//"_"
        elseif (model_name .eq. "SLL_CIRCULAR_ADV") then
           base_name = "ciradv_nc"//trim(str_num_cells)//"_"
        else
           base_name = "nc"//trim(str_num_cells)//"_"
        end if
        dist_name = "dist_"//trim(base_name)//"_"
        dist_error_name = "dist_error_"//trim(base_name)//"_"
        dist_exact_name = "dist_exact_"//trim(base_name)//"_"
        thdiag_1d_filename = "thdiag_1d"//trim(base_name)//".dat"
     endif
     ...
     dist_max_loc = maxval(dist_tn)
     dist_min_loc = minval(dist_tn)
     dist_max = dist_max_loc
     ...
     do step=1,number_iterations
        ...
        do i=1, n_points
           ...
           h1 =  xx*r11 + yy*r12
           h2 =  xx*r21 + yy*r22
           rad_st = radius - delta ! radius minus a mesh step
           if (abs(h1) > rad_st .or. abs(h2) > rad_st) inside = .false.
           if (abs(xx) > (rad_st)*sll_p_sqrt3*0.5_f64) inside = .false.
           if ( inside ) then
        ...
        if(modulo(step,freq_diag_time)==0)then
           ...
           dist_max_loc = maxval(dist_tn)
           dist_max = max(dist_max_loc,dist_max)
           ...
           print *,'# time, diag_number.........................',time_t, count
           print *,'    distribution min and max: ', dist_min, dist_max
           print *,'    err l1, l2, linf:', l1_err_loc, l2_err_loc, linf_err_loc
           ...
           end if ! if last num_cells........
        endif ! if write diags.............
     ...
     SLL_DEALLOCATE_ARRAY(dist_tn,ierr)
     SLL_DEALLOCATE_ARRAY(dist_tn1,ierr)
     SLL_DEALLOCATE_ARRAY(dist_exact,ierr)

  enddo ! End Loop for num_cells.....................
  ...
  subroutine gaussian_at_t( &
    ...
    do i = 1,mesh%num_pts_tot
       ...
       case(SLL_CIRCULAR_ADV)
          ...
          xx = r * cos(-2._f64 * sll_p_pi * time_t + th)
          yy = r * sin(-2._f64 * sll_p_pi * time_t + th)
       case(SLL_LINEAR_ADV)
          xx = x - a0 * time_t
          yy = y - a1 * time_t
       case default
       ...
       ! xx = x*cos(time_t) - y*sin(time_t)
       ! yy = x*sin(time_t) + y*cos(time_t)

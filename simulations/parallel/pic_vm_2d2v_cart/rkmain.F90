...
sll_real64 :: xmin,Et1(4,120000)!,Et2(4,200000),Et3(4,200000),Et4(4,200000)
sll_real64 :: xmax
sll_real64 :: ymin
sll_real64 :: ymax
...
!----Leap-frog----
    xtemp(1,:)=xmin+p%dpx+p%idx*dx
    xtemp(2,:)=ymin+p%dpy+p%idy*dy
    Et1(1,:)=p%vpx/ep
    Et1(2,:)=p%vpy/ep
    ...
    Et1(1,:)=vt(1,:)/ep
    Et1(2,:)=vt(2,:)/ep
    Et1(3,:)=p%epx/ep+vt(2,:)/ep**2
    Et1(4,:)=p%epy/ep-vt(1,:)/ep**2
    do m=1,nbpart
        vt(1,m)=p%vpx(m)+dt*Et1(3,m)
        vt(2,m)=p%vpy(m)+dt*Et1(4,m)
        xt(1,m)=xtemp(1,m)+dt*Et1(1,m)
        xt(2,m)=xtemp(2,m)+dt*Et1(2,m)
        call apply_bc()
        p%idx(m) = floor((xt(1,m)-xmin)/dimx*nx)
        p%dpx(m) = real(xt(1,m) -xmin- p%idx(m)*dx, f32)
        p%idy(m) = floor((xt(2,m)-ymin)/dimy*ny)
...
!        call apply_bc()
!        p%idx(m) = floor((xt(1,m)-xmin)/dimx*nx)
!        p%dpx(m) = real((xt(1,m) -xmin)/dx- p%idx(m), f32)
!        p%idy(m) = floor((xt(2,m)-ymin)/dimy*ny)
!        p%dpy(m) = real((xt(2,m) -ymin)/dy- p%idy(m), f32)
!    enddo
!    call interpol_eb( f, p )
!Et2(1,:)=vt(1,:)/ep
!Et2(2,:)=vt(2,:)/ep

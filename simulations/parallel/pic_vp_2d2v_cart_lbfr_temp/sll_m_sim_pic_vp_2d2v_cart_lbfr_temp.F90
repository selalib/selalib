...
module sll_m_sim_pic_vp_2d2v_cart_lbfr_temp
...
#include "sll_particle_representation.h"
  ...
  use sll_m_pic_lbfr_4d_group, only: &
    ...
    sll_s_new_pic_lbfr_4d_group_ptr  !  replaces sll_f_pic_lbfr_4d_group_new
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  ...
  type, extends(sll_c_simulation_base_class) :: sll_t_sim_pic_vp_2d2v_cart_lbfr_temp
  ...
  end type sll_t_sim_pic_vp_2d2v_cart_lbfr_temp
...
contains
  ...
  subroutine particles_snapshot(time,sim)
    ...
    class(sll_t_sim_pic_vp_2d2v_cart_lbfr_temp), intent(in) :: sim
  ...
  subroutine init_4d_generic_pic_cartesian   ( sim, filename )
    intrinsic :: trim
    class(sll_t_sim_pic_vp_2d2v_cart_lbfr_temp), intent(inout) :: sim
...
!    SLL_ALLOCATE( sim%pic_visualizer_interface, ierr )
    ...
    if( sim%use_pic_lbfr_scheme )then
      ...
      ! [[construct a particle_group of pic_lbfr type]]
    ...
    sim%n_particles = sim%particle_group%n_particles    ! with pic_lbfr this is the number of flow markers and deposition particles
  ...
  subroutine run_4d_generic_pic_cartesian( sim )
    ...
    !>     sll_t_sim_pic_vp_2d2v_cart_lbfr_temp::particle_group
    !>
    !> - we end with
    ...
    do it = 0, sim%n_iterations-1
      ...
      do k = 1, sim%n_particles
         ...
         if( .not. sll_f_x_is_in_domain_2d( coords(1), coords(2), sim%mesh_2d,            &
      ...
      if (sim%my_rank == 0 .and. mod(it, sim%plot_period)==0 ) then
        ...
        call sll_s_visualize_particle_group( sim%particle_group, plotting_params_2d, it+1)
...
end module sll_m_sim_pic_vp_2d2v_cart_lbfr_temp

...
!time_iterations
      ...
      case ("SLL_FFT")
         sim%poisson => sll_f_new_poisson_1d_periodic(x1_min, x1_max, nc_x1)
      case ("SLL_POLAR")
...
!$    tid = omp_get_thread_num() + 1
      ...
      case ("SLL_NON_UNIFORM_CUBIC_SPLINES") ! arbitrary order sll_p_lagrange periodic interpolation
         sim%sp(2)%advect_x2(tid)%ptr => sll_f_new_advector_1d_non_uniform_cubic_splines( &
                                         num_cells_x2_sp2, &
                                         x2_min_sp2, &
                                         x2_max_sp2, &
                                         order_x2_sp2, &
      ...
      case ("SLL_LAGRANGE") ! arbitrary order sll_p_lagrange periodic interpolation
         sim%sp(1)%advect_x2(tid)%ptr => sll_f_new_periodic_1d_advector( &
                                         ...
                                         order_x2_sp1)
      case ("SLL_NON_UNIFORM_CUBIC_SPLINES") ! arbitrary order sll_p_lagrange periodic interpolation
         sim%sp(1)%advect_x2(tid)%ptr => sll_f_new_advector_1d_non_uniform_cubic_splines( &
                                         num_cells_x2_sp1, &
                                         x2_min_sp1, &
                                         x2_max_sp1, &
                                         order_x2_sp1, &

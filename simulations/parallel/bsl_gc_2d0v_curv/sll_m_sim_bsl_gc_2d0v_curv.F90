...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  ...
  type, extends(sll_c_simulation_base_class) :: &
   ...
   ! Simulation should carry the pointers to the heap-allocated objects itself
    ...
    class(sll_c_characteristics_2d_base), pointer :: charac2d => null()
    class(sll_c_characteristics_1d_base), pointer :: charac1d_x1 => null()
    class(sll_c_characteristics_1d_base), pointer :: charac1d_x2 => null()
...
!   class(sll_c_interpolator_2d), pointer   :: phi_interp2d
...
#ifdef MUDPACK
    type(sll_t_mudpack_2d) :: poisson2
#endif    
   !time_iterations
   sll_real64 :: dt
  ...
  end type sll_t_simulation_2d_guiding_center_curvilinear

contains

  function sll_f_new_guiding_center_2d_curvilinear( &
    filename, &
    num_run &
    ) result(sim)
    type(sll_t_simulation_2d_guiding_center_curvilinear), pointer :: sim 
  ...
  end function sll_f_new_guiding_center_2d_curvilinear
  
  subroutine initialize_guiding_center_2d_curvilinear( &
    sim, &
    filename, &
    num_run)
    class(sll_t_simulation_2d_guiding_center_curvilinear), intent(inout) :: sim
    character(len=*), intent(in), optional :: filename
    sll_int32, intent(in), optional :: num_run
    sll_int32             :: IO_stat
...
#ifdef MUDPACK
    print *,'#MUDPACK IS ON'
    mudpack_method = sll_p_non_separable_without_cross_terms  
#else
    mudpack_method = 0
#endif
...
!    eps = 1.e-6_f64
    ...
    sim%quadrature_type1 = sll_p_es_gauss_legendre
    sim%quadrature_type2 = sll_p_es_gauss_legendre
...
!        sim%bc_interp2d_eta2= sll_p_dirichlet
    ...
    select case (mesh_case)
      ...
      case ("SLL_LANDAU_MESH")
        eta1_max = 2._f64 *sll_p_pi/kmode_eta1
        eta2_max = 2._f64 *sll_p_pi/kmode_eta2
      case ("SLL_POLAR_MESH")
        eta2_max = 2._f64*sll_p_pi
      ...
      case ("SLL_D_SHAPED_MESH")
        ...
        eta2_max = eta2_max
         ...
         !             cos(2._f64*sll_p_pi*eta2+alpha4*sin(2._f64*sll_p*eta2))
         !        x2 = alpha5*(alpha2*(2._f64*eta1-1)+alpha3)*sin(2._f64*sll_p_pi*eta2)
         ! Domain: [0,1] X [0,1]
    ...
    select case (f_interp2d_case)
      case ("SLL_CUBIC_SPLINES")
        ...
        sim%f_interp2d => sll_f_new_cubic_spline_interpolator_2d( &
          Nc_eta1+1, &
          Nc_eta2+1, &
          eta1_min, &
          eta1_max, &
    ...
    select case (A_interp_case)
      case ("SLL_CUBIC_SPLINES")
       print*,"#A1_2d interpolation SLL_CUBIC_SPLINES"
        sim%A1_interp2d => sll_f_new_cubic_spline_interpolator_2d( &
          Nc_eta1+1, &
          Nc_eta2+1, &
          eta1_min, &
          eta1_max, &
       ...
       print*,"#A2_2d interpolation SLL_CUBIC_SPLINES"   
        sim%A2_interp2d => sll_f_new_cubic_spline_interpolator_2d( &
          Nc_eta1+1, &
          Nc_eta2+1, &
          eta1_min, &
          eta1_max, &
       ...
       print*,"#A1_1d interpolation SLL_CUBIC_SPLINES"   
        sim%A1_interp1d_x1 => sll_f_new_cubic_spline_interpolator_1d( &
          Nc_eta1+1, &
          eta1_min, &
          eta1_max, &
          sim%bc_interp2d_eta1, &
          slope_left = 0._f64, &
          slope_right = 0._f64)
        sim%A1_interp1d_x2 => sll_f_new_cubic_spline_interpolator_1d( &
          Nc_eta2+1, &
          eta2_min, &
          eta2_max, &
          sim%bc_interp2d_eta2, &
       ...
       print*,"#A2_1d interpolation SLL_CUBIC_SPLINES"     
        sim%A2_interp1d_x1 => sll_f_new_cubic_spline_interpolator_1d( &
          Nc_eta1+1, &
          eta1_min, &
          eta1_max, &
          sim%bc_interp2d_eta1, &
          slope_left = 0._f64, &
          slope_right = 0._f64)
        sim%A2_interp1d_x2 => sll_f_new_cubic_spline_interpolator_1d( &
          Nc_eta2+1, &
          eta2_min, &
          eta2_max, &
          sim%bc_interp2d_eta2, &
    ...
    select case (phi_interp2d_case)
      ...
      print*,"#phi interpolation SLL_CUBIC_SPLINES"  
        sim%phi_interp2d => sll_f_new_cubic_spline_interpolator_2d( &
          Nc_eta1+1, &
          Nc_eta2+1, &
          eta1_min, &
          eta1_max, &
    ...
    select case (f_interp1d_x1_case)
      case ("SLL_CUBIC_SPLINES")
        sim%f_interp1d_x1 => sll_f_new_cubic_spline_interpolator_1d( &
          Nc_eta1+1, &
          eta1_min, &
          eta1_max, &
          sim%bc_interp2d_eta1, &
    ...
    select case (f_interp1d_x2_case)
      case ("SLL_CUBIC_SPLINES")
        sim%f_interp1d_x2 => sll_f_new_cubic_spline_interpolator_1d( &
          Nc_eta2+1, &
          eta2_min, &
          eta2_max, &
          sim%bc_interp2d_eta2, &
    ...
    select case(advect1d_x1_case)
      case ("SLL_BSL")
        sim%advect_1d_x1 => sll_f_new_bsl_1d_advector(&
          ...
          eta_max = eta1_max)
      case ("SLL_CSL_PERIODIC")
        sim%advect_1d_x1 => sll_f_new_csl_periodic_1d_advector(&
          sim%f_interp1d_x1, &
          sim%charac1d_x1, &
          Nc_eta1+1, &
          eta_min = eta1_min, &
    ...
    select case(advect1d_x2_case)
      case ("SLL_BSL")
        sim%advect_1d_x2 => sll_f_new_bsl_1d_advector(&
          ...
          eta_max = eta2_max)
      case ("SLL_CSL_PERIODIC")
        sim%advect_1d_x2 => sll_f_new_csl_periodic_1d_advector(&
          sim%f_interp1d_x2, &
          sim%charac1d_x2, &
          Nc_eta2+1, &
          eta_min = eta2_min, &
    ...
    select case(charac2d_case)
      case ("SLL_EULER")
        ...
        sim%charac2d => sll_f_new_explicit_euler_2d_charac(&
          Nc_eta1+1, &
          Nc_eta2+1, &
          eta1_min=eta1_min, &
          eta1_max=eta1_max, &
      ...
      case ("SLL_VERLET")    
        ...
        sim%charac2d => sll_f_new_verlet_2d_charac(&
          Nc_eta1+1, &
          Nc_eta2+1, &
          sim%A1_interp2d, &
          sim%A2_interp2d, &
    ...
    select case(initial_function_case)
      ...
      case("SLL_DIOCOTRON")
        ...
        sim%init_func => sll_f_diocotron_initializer_2d2
       ...
       case ("sll_f_dsg_2d")
        print*,"#f0 = sll_f_dsg_2d"  
        sim%init_func => sll_f_dsg_2d
        SLL_ALLOCATE(sim%params(4),ierr)
        ...
        sim%params(2) = r_plus
        sim%params(3) = eps
        sim%params(4) = kmode_eta2 *2._f64*sll_p_pi 
      case default
    ...
    select case(poisson_solver)    
      ...
      case ("SLL_MUDPACK_CURVILINEAR")    
       ...
       case("SLL_ELLIPTIC_FINITE_ELEMENT_SOLVER")
        ...
        sim%b1  = 0._f64
        sim%b2  = 0._f64
        if ((sim%bc_eta1_left == sll_p_periodic .and. sim%bc_eta1_right == sll_p_periodic .and. &
             sim%bc_eta2_left== sll_p_periodic .and. sim%bc_eta2_right== sll_p_periodic).or. &
             (sim%bc_eta1_left == sll_p_neumann .and. sim%bc_eta1_right == sll_p_neumann .and. &
             sim%bc_eta2_left== sll_p_neumann .and. sim%bc_eta2_right== sll_p_neumann)) then
        ...
        sim%poisson => sll_f_new_poisson_2d_elliptic_solver( &
         sim%transformation,&
...
!        do i1=1,Nc_eta1+1
!          eta1=eta1_min+real(i1-1,f64)*delta_eta1
!         write(201,*) eta1,eta2,phi(i1,i2),sin(2.0*sll_p_pi*eta1)*sin(2.0*sll_p_pi*eta2)
!        end do
!     end do   
!     stop  
    ...
    !call sll_s_ascii_file_create('thdiag.dat', thdiag_id, ierr)
    call sll_s_ascii_file_create(sim%thdiagp_filename, thdiagp_id, ierr)
    iplot = 0
...
subroutine compute_field_from_phi_2d_fd_curvilinear(phi,mesh_2d,transformation,A1,A2,interp2d,d1,d2)
  ...
  subroutine time_history_diagnostic_collela( &
    ...
    do i2 = 1, Nc_eta2+1
      ...
      do i1=1,Nc_eta1+1
        ...
        !/abs(transformation%jacobian(eta1,eta2)) 
      enddo
      array(i2) = sll_f_compute_integral_trapezoid_1d(data, Nc_eta1+1, delta_eta1)
    ...
    val = sll_f_compute_integral_trapezoid_1d(array, Nc_eta2+1, delta_eta2)
  ...
  subroutine time_history_diagnostic_polar( &
    ...
    do i2 = 1, Nc_eta2
      ...
      enddo
      w = w + sll_f_compute_integral_trapezoid_1d(data, Nc_eta1+1, delta_eta1)

      do i1=1,Nc_eta1+1
        eta1 = eta1_min+real(i1-1,f64)*delta_eta1
        data(i1) = eta1*abs(f(i1,i2))
      enddo
      l1 = l1 + sll_f_compute_integral_trapezoid_1d(data, Nc_eta1+1, delta_eta1)

      do i1=1,Nc_eta1+1
        eta1 = eta1_min+real(i1-1,f64)*delta_eta1
        data(i1) = eta1*(f(i1,i2))**2
      enddo
      l2 = l2 + sll_f_compute_integral_trapezoid_1d(data, Nc_eta1+1, delta_eta1)

      do i1=1,Nc_eta1+1
        eta1 = eta1_min+real(i1-1,f64)*delta_eta1
        data(i1) = eta1*((eta1*A2(i1,i2))**2+A1(i1,i2)**2)
      enddo
      e = e + sll_f_compute_integral_trapezoid_1d(data, Nc_eta1+1, delta_eta1)

      do i1=1,Nc_eta1+1
        eta1 = eta1_min+real(i1-1,f64)*delta_eta1
        data(i1) = eta1*phi(i1,i2)
      enddo
      int_r(i2) = sll_f_compute_integral_trapezoid_1d(data, Nc_eta1+1, delta_eta1)     
    enddo     
...
!*********************
  ...
  subroutine plot_f_curvilinear(iplot,f,mesh_2d,transf)
    ...
    call sll_o_xdmf_open("f"//cplot//".xmf","curvilinear_mesh", &
  ...
  subroutine plot_phi_curvilinear(iplot,f,mesh_2d,transf)
    ...
    call sll_s_int2string(iplot,cplot)
    call sll_o_xdmf_open("phi"//cplot//".xmf","curvilinear_mesh", &
      nnodes_x1,nnodes_x2,file_id,error)
    call sll_o_xdmf_write_array("phi"//cplot,f,"values", &
      error,file_id,"Node")
    call sll_s_xdmf_close(file_id,error)
...
subroutine sll_DSG( eta1_min,eta1_max, eta2_min,eta2_max,n_eta1,n_eta2, f ) 
    ...
    !type(sll_t_arbitrary_degree_spline_interpolator_2d)   :: a11_interp
    !type(sll_t_arbitrary_degree_spline_interpolator_2d)   :: a22_interp
    !type(sll_t_arbitrary_degree_spline_interpolator_2d)   :: a12_interp
    type(sll_t_cubic_spline_interpolator_2d)   :: a11_interp
    type(sll_t_cubic_spline_interpolator_2d)   :: a22_interp
    type(sll_t_cubic_spline_interpolator_2d)   :: a12_interp
    ...
    do j= 1, n_eta2+1
      ...
      do i= 1, n_eta1+1
         ...
         jac_12 = -(0.148*eta1 + 0.462_f64)*sin(pi2*eta2 + 0.4290421957_f64* &
              sin(pi2*eta2))*(pi2 + 0.8580843914_f64*cos(pi2*eta2)*sll_p_pi)
         jac_21 = 0.24568_f64*sin(pi2*eta2) 
         jac_22 = 3.32_f64*sll_p_pi*(0.148_f64*eta1 + 0.462_f64)*cos(pi2*eta2)
  ...
  subroutine sll_s_delete_guiding_center_2d_curvilinear( sim )

    class(sll_t_simulation_2d_guiding_center_curvilinear) :: sim
            
  end subroutine sll_s_delete_guiding_center_2d_curvilinear
  ...
  function compute_integral_trapezoid_2d( &
    ...
    do i=1,Npts2
      array(i) = sll_f_compute_integral_trapezoid_1d( &
        input(1:Npts1,i),&
        Npts1, &
        delta1)
    enddo    
    res = sll_f_compute_integral_trapezoid_1d( &
      array,&
      Npts2, &
      delta2)

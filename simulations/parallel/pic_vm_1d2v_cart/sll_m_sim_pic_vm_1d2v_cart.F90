...
#include "sll_working_precision.h"
  ...
  use sll_m_time_propagator_pic_vm_1d2v_disgrad, only: &
       sll_t_time_propagator_pic_vm_1d2v_disgrad

  use sll_m_time_propagator_pic_vm_1d2v_dgsub, only: &
       sll_t_time_propagator_pic_vm_1d2v_dgsub
  ...
  sll_int32, parameter :: sll_p_splitting_disgrad = 10
  sll_int32, parameter :: sll_p_splitting_disgradsub = 13
...
contains
  ...
  subroutine init_pic_vm_1d2v (sim, filename)
    ...
    case("splitting_disgrad")
       sim%splitting_case = sll_p_splitting_disgrad
    case("splitting_disgradsub")
       sim%splitting_case = sll_p_splitting_disgradsub
    ...
    case("disgrad")
       sim%make_ctest = .true.
       sim%ctest_ref_file_rho = "ctests/reffile_disgrad_pic_vm_1d2v_cart_rho.dat"
       sim%ctest_ref_file_thdiag = "ctests/reffile_disgrad_pic_vm_1d2v_cart_thdiag.dat"
    case("disgrad_stronga")
       sim%make_ctest = .true.
       sim%ctest_ref_file_rho = "ctests/reffile_disgrad_stronga_pic_vm_1d2v_cart_rho.dat"
       sim%ctest_ref_file_thdiag = "ctests/reffile_disgrad_stronga_pic_vm_1d2v_cart_thdiag.dat"
    case("disgradsub")
       sim%make_ctest = .true.
       sim%ctest_ref_file_rho = "ctests/reffile_disgradsub_pic_vm_1d2v_cart_rho.dat"
       sim%ctest_ref_file_thdiag = "ctests/reffile_disgradsub_pic_vm_1d2v_cart_thdiag.dat"
    case("hs_smooth")
       sim%make_ctest = .true.
    ...
    case("disgrad_smooth")
       sim%make_ctest = .true.
       sim%ctest_ref_file_rho = "ctests/reffile_disgrad_smooth_pic_vm_1d2v_cart_rho.dat"
       sim%ctest_ref_file_thdiag = "ctests/reffile_disgrad_smooth_pic_vm_1d2v_cart_thdiag.dat"
    case("disgradsub_smooth")
       sim%make_ctest = .true.
       sim%ctest_ref_file_rho = "ctests/reffile_disgradsub_smooth_pic_vm_1d2v_cart_rho.dat"
       sim%ctest_ref_file_thdiag = "ctests/reffile_disgradsub_smooth_pic_vm_1d2v_cart_thdiag.dat"
    case("trafo")
       sim%make_ctest = .true.
...
!!$          end select
    ...
    elseif  (sim%splitting_case == sll_p_splitting_disgrad) then
       allocate( sll_t_time_propagator_pic_vm_1d2v_disgrad :: sim%propagator )
       select type( qpdisgradEd=>sim%propagator )
       type is ( sll_t_time_propagator_pic_vm_1d2v_disgrad )
          call qpdisgradEd%init( sim%maxwell_solver, &
               sim%kernel_smoother_0, sim%kernel_smoother_1, sim%particle_group, &
               sim%efield_dofs, sim%bfield_dofs, &
               sim%domain(1), sim%domain(3), sim%filter, trim(filename) )
          sim%efield_dofs_n => qpdisgradEd%helper%efield_dofs
       end select
      elseif  (sim%splitting_case == sll_p_splitting_disgradsub) then
       allocate( sll_t_time_propagator_pic_vm_1d2v_dgsub :: sim%propagator )
       select type( qpdgs=>sim%propagator )
       type is ( sll_t_time_propagator_pic_vm_1d2v_dgsub )
...
!!$       end if
  ...
  subroutine add_background_charge(sim, background_charge)
    ...
    allocate(spline_val( sim%degree_fem+1, q ))
    quad_xw = sll_f_gauss_legendre_points_and_weights( q , 0.0_f64, 1.0_f64 )

    do i = 1, q
       call sll_s_uniform_bsplines_eval_basis( sim%degree_fem, quad_xw(1,i), spline_val(:,i) )
    end do

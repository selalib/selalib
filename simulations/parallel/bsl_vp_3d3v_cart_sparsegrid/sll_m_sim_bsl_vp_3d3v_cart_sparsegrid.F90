...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  ...
  type, extends(sll_c_simulation_base_class) :: sll_t_sim_sl_vp_3d3v_cart_sparsegrid
     logical     :: is_mdeltaf
     sll_int32   :: test_case
     sll_int32   :: levelx
     sll_int32   :: levelv
     ...
     ! Domain
     sll_real64  :: eta_min(6), eta_max(6)
     ...
     sll_int32, dimension(3) :: levelsx,levelsv, dorder1, dorder2, dorder3
     ...
     ! Stabilization
     sll_int32 :: switch_step

     !Diagnostics
     sll_real64, dimension(:), allocatable :: nrj

     contains
       procedure :: init_from_file => init_sparsegrid_3d3v
       procedure :: run => run_sparsegrid_3d3v
...
!------------------------------------------------------------------------------!
  subroutine init_sparsegrid_3d3v (sim, filename)
    ...
    if (io_stat /= 0) then
       print*, 'init_sparsegrid_3d3v() failed to open file', filename
       STOP
    end if
  ...
  end subroutine init_sparsegrid_3d3v

!------------------------------------------------------------------------------!
  subroutine run_sparsegrid_3d3v(sim)
    class(sll_t_sim_sl_vp_3d3v_cart_sparsegrid), intent(inout) :: sim
    ...
    !Sparse grid
    sim%levelsx(1) = sim%levelx; sim%levelsx(2:3) = sim%levelsx(1);
    sim%levelsv(1) = sim%levelv; sim%levelsv(2:3) = sim%levelsv(1);
    ...
    elseif (sim%test_case  == SLL_TSI) then
       ...
       sim%eta_min(3) =  0.0_f64; sim%eta_max(3) =  4.0_f64 * sll_p_pi
       !v domain
    ...
    ! Set initial value
    do j=1,3
       kxyz(j)  = 2.0_f64*sll_p_pi/(sim%eta_max(j)-sim%eta_min(j))
    end do

    do i1=1,sim%local_size_x(1)
       eta(1:3) =  sim%interp_x%hierarchy(sim%global_x(1)+i1)%coordinate
       do i2=1,sim%local_size_x(2)
          eta(4:6) =sim%interp_v%hierarchy(sim%global_x(2)+i2)%coordinate
          sim%f_x(i1,i2) = 1.0_f64/(2.0_f64*sll_p_pi)**(1.5_f64)*&
               (1.0_f64+sim%eps*(cos(kxyz(1)*eta(1))+&
               cos(kxyz(2)*eta(2))+cos(kxyz(3)*eta(3))))
    ...
    call sim%poisson%solve(sim%interp_x,sim%rho,sim%ex,sim%ey,sim%ez)
  ...
  subroutine advection_x(sim, dt)
    ...
    do i2 = 1, sim%local_size_x(2)
       ...
       call sim%interp_x%interpolate_const_disp(sim%dorder2,&
            ...
            sim%f2_x(:,i2), sim%f_x(:,i2),.TRUE.)
       call sim%interp_x%interpolate_const_disp(sim%dorder3,&
            -dt*sim%interp_v%hierarchy(sim%global_x(2)+i2)%coordinate(3),&
            sim%f_x(:,i2), sim%f2_x(:,i2),.FALSE.)
    end do
    sim%f_x = sim%f2_x;
  ...
  subroutine advection_v(sim, dt)
    ...
    do i1 = 1, sim%local_size_v(1) 
       ...
       call sim%interp_v%interpolate_const_disp(sim%dorder2,&
            dt*sim%ey(sim%global_v(1)+i1),sim%ft2_v(:,i1), sim%ft_v(:,i1),.TRUE.);
  ...
  subroutine compute_rho_df(sim)
    ...
    do i1 = 1, sim%local_size_v(1)
       ...
       do i2 = 1, sim%local_size_v(2)
          ...
          if (sim%interp_v%hierarchy(sim%global_v(2)+i2)%level(3) == 0) then
             phi(3) = sqrt(2.0_f64*sll_p_pi) 
          else
             h = sim%interp_v%length(3)/&
                  (real(2**sim%interp_v%hierarchy(sim%global_v(2)+i2)%level(3),f64));
             x0 = sim%interp_v%hierarchy(sim%global_v(2)+i2)%coordinate(3)
             phi(3) = int_f0_alpha(x0,h,0.5_f64);
             !phi(3) = 0.0_f64
          end if
...
end module sll_m_sim_sl_vp_3d3v_cart_sparsegrid

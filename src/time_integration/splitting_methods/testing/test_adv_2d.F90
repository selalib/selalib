...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  ...
  type(sll_t_cubic_spline_interpolator_1d), target  :: spl1
  type(sll_t_cubic_spline_interpolator_1d), target  :: spl2
  class(sll_c_characteristics_1d_base),     pointer :: charac1
  class(sll_c_characteristics_1d_base),     pointer :: charac2
  type(sll_t_charac_1d_explicit_euler),     target  :: euler_1
  type(sll_t_charac_1d_explicit_euler),     target  :: euler_2
  ...
  call spl1%init( num_cells_x1+1, x1_min, x1_max, sll_p_periodic)
  call spl2%init( num_cells_x2+1, x2_min, x2_max, sll_p_periodic)

  interp1 => spl1
  interp2 => spl2

  call euler_1%init( num_cells_x1+1, sll_p_periodic)
  call euler_2%init( num_cells_x2+1, sll_p_periodic)

  charac1 => euler_1
  charac2 => euler_2
  
  call split%init(  f,  A1,  A2, &
      interp1, charac1, process_outside_point1, &
      interp2, charac2, process_outside_point2, &
      mesh_2d, sll_p_advective, sll_p_strang_tvt) 

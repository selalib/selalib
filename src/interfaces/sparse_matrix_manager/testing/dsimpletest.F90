!
!  This file is part of MUMPS 5.0.1, released
!  on Thu Jul 23 17:08:29 UTC 2015
!
PROGRAM MUMPS_TEST
use mpi
implicit none
INCLUDE 'dmumps_struc.h'
TYPE (DMUMPS_STRUC) mumps_par
INTEGER IERR, I
character(len=272) :: input_file
integer :: input_file_unit


CALL MPI_INIT(IERR)
! Define a communicator for the package.
mumps_par%COMM = MPI_COMM_WORLD
!  Initialize an instance of the package
!  for L U factorization (sym = 0, with working host)
mumps_par%JOB = -1
mumps_par%SYM = 0
mumps_par%PAR = 1
CALL DMUMPS(mumps_par)
IF (mumps_par%INFOG(1).LT.0) THEN
 WRITE(6,'(A,A,I6,A,I9)') " ERROR RETURN: ", &
            "  mumps_par%INFOG(1)= ", mumps_par%INFOG(1),  &
            "  mumps_par%INFOG(2)= ", mumps_par%INFOG(2) 
 GOTO 500
END IF
!  Define problem on the host (processor 0)
IF ( mumps_par%MYID .eq. 0 ) THEN
  CALL GET_COMMAND_ARGUMENT(1 , input_file)
  print*, trim(input_file)
  open(newunit = input_file_unit, file=trim(input_file))
  READ(input_file_unit,*) mumps_par%N
  READ(input_file_unit,*) mumps_par%NZ
  ALLOCATE( mumps_par%IRN ( mumps_par%NZ ) )
  ALLOCATE( mumps_par%JCN ( mumps_par%NZ ) )
  ALLOCATE( mumps_par%A( mumps_par%NZ ) )
  ALLOCATE( mumps_par%RHS ( mumps_par%N  ) )
  DO I = 1, mumps_par%NZ
    READ(input_file_unit,*) mumps_par%IRN(I),mumps_par%JCN(I),mumps_par%A(I)
  END DO
  DO I = 1, mumps_par%N
    READ(input_file_unit,*) mumps_par%RHS(I)
  END DO
END IF
!  Call package for solution
mumps_par%JOB = 6
CALL DMUMPS(mumps_par)
IF (mumps_par%INFOG(1).LT.0) THEN
 WRITE(6,'(A,A,I6,A,I9)') " ERROR RETURN: ", &
            "  mumps_par%INFOG(1)= ", mumps_par%INFOG(1),  &
            "  mumps_par%INFOG(2)= ", mumps_par%INFOG(2) 
 GOTO 500
END IF
!  Solution has been assembled on the host
IF ( mumps_par%MYID .eq. 0 ) THEN
  WRITE( 6, * ) ' Solution is ',(mumps_par%RHS(I),I=1,mumps_par%N)
END IF
!  Deallocate user data
IF ( mumps_par%MYID .eq. 0 )THEN
  DEALLOCATE( mumps_par%IRN )
  DEALLOCATE( mumps_par%JCN )
  DEALLOCATE( mumps_par%A   )
  DEALLOCATE( mumps_par%RHS )
END IF
!  Destroy the instance (deallocate internal data structures)
mumps_par%JOB = -2
CALL DMUMPS(mumps_par)
IF (mumps_par%INFOG(1).LT.0) THEN
 WRITE(6,'(A,A,I6,A,I9)') " ERROR RETURN: ", &
            "  mumps_par%INFOG(1)= ", mumps_par%INFOG(1),  &
            "  mumps_par%INFOG(2)= ", mumps_par%INFOG(2) 
 GOTO 500
END IF
500  CALL MPI_FINALIZE(IERR)
STOP
END

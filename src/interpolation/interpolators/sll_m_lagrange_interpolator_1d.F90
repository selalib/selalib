...
private
 ...
 type,extends(sll_c_interpolator_1d), public :: sll_lagrange_interpolator_1d
   ...
   !> PLEASE ADD DOCUMENTATION
   procedure :: interpolate_from_interpolant_derivatives_eta => interpolate_array_derivatives_li1d
   !> PLEASE ADD DOCUMENTATION
...
subroutine interpolate_array_disp_li1d(this, num_pts, data, alpha, output_array)
  ...
  sll_real64, dimension(1:num_pts), intent(inout) :: data  ! data to be interpolated points where output is desired
  sll_real64, dimension(1:num_pts), intent(inout)    :: output_array

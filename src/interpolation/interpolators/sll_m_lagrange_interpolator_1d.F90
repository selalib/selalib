...
subroutine interpolate_array_disp_li1d(this, num_pts, data, alpha, output_array)
  ...
  case (SLL_D_INTERP_LAGRANGE_FIXED)
     ...
     case (SLL_PERIODIC)
        call sll_s_interpolate_array_disp_lagrange_fixed_periodic(data, output_array, alpha, this%stencil_width)
     case (SLL_D_ONE_SIDED)
        call sll_s_interpolate_array_disp_lagrange_fixed_no_bc(data, output_array, alpha, this%stencil_width)
     case (SLL_D_HALO)
        call sll_s_interpolate_array_disp_lagrange_fixed_halo_cells(data, output_array, alpha, this%stencil_width)
     case default

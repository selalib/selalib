...
!> The module sll_m_lagrange_fast is a fast implementation that also works in combination with domain decomposition. In particular, for the MPI domain decomposition use case, it implements a Lagrange interpolation that is always centered around the point that is displaced (odd number of points).
...
!> For the boundary closure, three possiblities are implemented: Either halo cells are provided or periodic boundary conditions can be used. A third alternative is to use a one-sided stencil at the boundary.
!> @todo

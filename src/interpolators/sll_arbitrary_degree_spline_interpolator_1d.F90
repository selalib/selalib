...
call compute_bspline_1d ( interpolator%bspline,     &
                          data_array,               &
                          interpolator%slope_left,  &
                          interpolator%slope_right  )


if (interpolator%bc_left == SLL_NEUMANN) then 
  interpolator%slope_left = 0.0_f64
end if

if (interpolator%bc_right == SLL_NEUMANN) then
  interpolator%slope_right = 0.0_f64
end if

if (interpolator%bc_left == SLL_DIRICHLET) then
   interpolator%coeff_splines(1)           = interpolator%value_left
end if

if (interpolator%bc_right == SLL_DIRICHLET) then
   interpolator%coeff_splines(sz+sz_deriv+k-1) = interpolator%value_right
end if
...
get_coefficients_ad1d => interpolator%bspline%bcoef
...
call update_bspline_1d(interpolator%bspline, vals_to_interpolate)

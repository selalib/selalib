...
#include "sll_errors.h"
  ...
  type, extends(operator_splitting) :: sll_operator_splitting_pic_2x2v_vp
     ...
     sll_real64, allocatable :: efield(:,:)

   contains
     procedure :: operatorT => advection_x_pic_2x2v_vp
     procedure :: operatorV => advection_v_pic_2x2v_vp
...
contains
  ...
  subroutine advection_x_pic_2x2v_vp(this, dt)
    ...
    !call this%particle_group%update_x(dt * this%particle_group%get_all_v)
    

  end subroutine advection_x_pic_2x2v_vp

  subroutine advection_v_pic_2x2v_vp(this, dt)
    class(sll_operator_splitting_pic_2x2v_vp), intent(inout) :: this !< object 
    sll_real64, intent(in) :: dt   !< time step
    ...
    !call this%particle_group%update_v(rhs)
  ...
  subroutine initialize_operator_splitting_pic_2x2v(this, poisson_solver, kernel_smoother, particle_group)
    ...
    SLL_ALLOCATE(this%rho_2d(this%kernel_smoother%n_grid(1), this%kernel_smoother%n_grid(2)), ierr)
    SLL_ALLOCATE(this%efield1(this%kernel_smoother%n_grid(1), this%kernel_smoother%n_grid(2)), ierr)   
    SLL_ALLOCATE(this%efield2(this%kernel_smoother%n_grid(1), this%kernel_smoother%n_grid(2)), ierr)
  ...
  function sll_new_splitting_pic_2x2v_vp(poisson_solver, kernel_smoother, particle_group) result(this)
    class(sll_operator_splitting_pic_2x2v_vp), pointer :: this !< object 
    class(poisson_2d_fft_solver),pointer, intent(in) :: poisson_solver
    class(sll_kernel_smoother_base),pointer, intent(in) :: kernel_smoother
    class(sll_particle_group_base),pointer, intent(in) :: particle_group

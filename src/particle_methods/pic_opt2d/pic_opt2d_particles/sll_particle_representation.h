...
#define GET_PARTICLE_POSITION(p,m2d,x,y) \
  do; \
     x = m2d%eta1_min + m2d%delta_eta1*(real(p%dx,f64) + real( mod(p%ic-1,m2d%num_cells1), f64) ); \
     y = m2d%eta2_min + m2d%delta_eta2*(real(p%dy,f64) + real( int( (p%ic-1)/m2d%num_cells1 ), f64)); \
    exit; \
 end do

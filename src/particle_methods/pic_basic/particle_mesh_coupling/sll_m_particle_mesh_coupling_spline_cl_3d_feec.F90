...
module sll_m_particle_mesh_coupling_spline_cl_3d_feec
...
#include "sll_working_precision.h"
  ...
  type, extends(sll_c_particle_mesh_coupling_3d) :: sll_t_particle_mesh_coupling_spline_cl_3d_feec
     ...
     sll_real64, allocatable :: spline_2d(:,:) !< scratch data for spline evaluation
     sll_real64, allocatable :: spline1_2d(:,:) !< scratch data for spline evaluation
     sll_real64, allocatable :: spline1_3d(:,:,:) !< scratch data for spline evaluation
     sll_real64, allocatable :: spline2_3d(:,:,:) !< scratch data for spline evaluation
     ...
     sll_int32              :: n_total0 !< product of the degrees of freedom in all three directions
     sll_int32              :: n_total1 !< product of the degrees of freedom in all three directions
   ...
   contains
     ...
     procedure :: add_charge => add_charge_single_spline_cl_3d_feec !> Add charge of one particle
     procedure :: add_particle_mass => add_particle_mass_spline_cl_3d_feec !> Collect diagonal entry of particle mass matrix
     procedure :: add_particle_mass_od => add_particle_mass_od_spline_cl_3d_feec !> Collect off-diagonal entry of particle mass matrix
     procedure :: evaluate => evaluate_field_single_spline_cl_3d_feec !> Evaluate spline function with given coefficients
     procedure :: evaluate_multiple => evaluate_multiple_spline_cl_3d_feec !> Evaluate multiple spline functions with given coefficients
     procedure :: add_current => add_current_cl_3d!> Add current via line integral
     procedure :: add_current_evaluate !>  Add current via line integral and evaluate spline
     procedure :: add_current_update_v_component1 => add_current_update_v_component1_spline_3d_feec !> Add contribution of one particle to the current density in x1 direction and update velocity
     procedure :: add_current_update_v_component2 => add_current_update_v_component2_spline_3d_feec !> Add contribution of one particle to the current density in x2 direction and update velocity
     procedure :: add_current_update_v_component3 => add_current_update_v_component3_spline_3d_feec !> Add contribution of one particle to the current density in x3 direction and update velocity

     procedure :: init => init_spline_cl_3d_feec !> Constructor
     procedure :: free => free_spline_cl_3d_feec !> Destructor
...
contains
  ...
  subroutine add_charge_single_spline_cl_3d_feec(self, position, marker_charge, degree, rho_dofs)
    class( sll_t_particle_mesh_coupling_spline_cl_3d_feec ), intent(inout)   :: self !< kernel smoother object
  ...
  subroutine update_jv(self, component, lower, upper, index, marker_charge, sign, bfield_dofs, start1, start2, stride, vi, j_dofs)
    class(sll_t_particle_mesh_coupling_spline_cl_3d_feec), intent(inout) :: self !< time splitting object 
    sll_int32,                              intent(in)    :: component
  ...
  !< Helper function to convert x 3d index to 1d
  ...
  !< Helper function to convert x to xbox 3d
  ...
  !< Helper function to convert x to xbox 1d
  ...
  !< Helper function to compute box indices
  ...
  subroutine init_spline_cl_3d_feec ( self, n_cells, domain, spline_degree, boundary, no_particles )
    class (sll_t_particle_mesh_coupling_spline_cl_3d_feec), intent( out ) :: self !< Kernel smoother object 
    ...
    sll_int32,                               intent(in) :: boundary(3) !< spline boundary conditions

...
#include "sll_working_precision.h"
  ...
  type, abstract :: sll_c_particle_group_base
   ...
   contains
     ...
     procedure( get_scalar ), deferred :: get_common_weight !< Get the mass of one particle
     ...
     procedure                           :: read !< Reads in particle array (default not working)
  ...
  type :: sll_t_particle_array
     ...
     sll_int32 :: n_species
     class(sll_c_particle_group_base), allocatable :: group(:)

  end type sll_t_particle_array
  ...
  !----------------------------------------------------------------------------
  abstract interface
     pure function get_scalar( self ) result(r)
       use sll_m_working_precision
       import sll_c_particle_group_base
       class( sll_c_particle_group_base ), intent( in ) :: self
       sll_real64 :: r
...
contains
  ...
  subroutine read( self, filename )
    class(sll_c_particle_group_base), intent ( inout ) :: self !< particle group object
    character(len=*), intent(in) :: filename

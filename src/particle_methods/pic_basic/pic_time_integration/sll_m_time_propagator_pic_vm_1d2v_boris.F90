...
contains
  ...
  subroutine lie_boris(self, dt, number_steps)
    class(sll_t_time_propagator_pic_vm_1d2v_boris), intent(inout) :: self !< time splitting object 
...
!!$               self%j_dofs_local(:,2))
 ...
 !---------------------------------------------------------------------------!
  ...
  subroutine initialize_pic_vm_1d2v_boris(&
       ...
       x_min, &
       Lx) 
    class(sll_t_time_propagator_pic_vm_1d2v_boris), intent(out) :: self !< time splitting object 

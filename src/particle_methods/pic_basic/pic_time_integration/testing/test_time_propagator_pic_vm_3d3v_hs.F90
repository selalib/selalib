...
program test_time_propagator_pic_3d3v_vm_hs
  !+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
#include "sll_working_precision.h"
  ...
  type(sll_t_time_propagator_pic_vm_3d3v_hs) :: propagator
  ...
  ! Reference fields 
  !------------------------------------------------
  reference_efield = "ref_hs_3d3v_efield"
  reference_bfield = "ref_hs_3d3v_bfield"
  ...
  call filter%init( 0, num_cells, [0,0,0] )

  call propagator%init( maxwell_solver, &
       particle_mesh_coupling, particle_group, &
       phi, efield, bfield, &
       domain(:,1), domain(:,3), filter )

  call propagator%operatorHp1( delta_t )
  call propagator%operatorHp2( delta_t )
  call propagator%operatorHp3( delta_t )

  ! Compare to reference
  ! Particle information after advect_x application 
  particle_info_check(:,1) = [5.737074171044746_f64, 2.764624640351017_f64, 1.15082423963525_f64, -1.53412054435254_f64, 0.15731068461017_f64, 1.62412057435254_f64, -5.7026946767517_f64] 
  ...
  ! Compare to reference
  ! Particle information after advect_ev application 
  particle_info_check(:,1) = [5.89048622548_f64, 2.74889357189_f64, 0.9884121822_f64, -1.546123499832182_f64, 0.18519746962949798_f64, 1.6164825294030409_f64, -5.7026946767517_f64]
  ! Compare computed values to reference values
  do i_part=1,n_particles
     xi = particle_group%group(1)%get_x(i_part)
     if( maxval(abs(xi-particle_info_check(1:3,i_part))) > EQV_TOL ) then
...
end program test_time_propagator_pic_3d3v_vm_hs

module sll_m_linear_operator_particle_mass_cl_3d_diag
#include "sll_working_precision.h"
  ...
  public :: sll_t_linear_operator_particle_mass_cl_3d_diag
  
  private

  type, extends(sll_c_particle_mass_3d_base) :: sll_t_linear_operator_particle_mass_cl_3d_diag
    
   contains
     procedure :: create => create_linear_operator_particle_mass_cl_3d_diag
     procedure :: free => free_particle_mass_cl_3d_diag
     procedure :: dot => dot_particle_mass_cl_3d_diag
     procedure :: print_info => print_info_particle_mass_cl_3d_diag

  end type sll_t_linear_operator_particle_mass_cl_3d_diag


contains

  subroutine create_linear_operator_particle_mass_cl_3d_diag( self,  n_cells, degree, degree2 )
    class(sll_t_linear_operator_particle_mass_cl_3d_diag), intent( inout ) :: self !< Particle mass
  ...
  end subroutine create_linear_operator_particle_mass_cl_3d_diag
  ...
  end subroutine free_particle_mass_cl_3d_diag
  
  
  subroutine dot_particle_mass_cl_3d_diag ( self, x, y )
    class(sll_t_linear_operator_particle_mass_cl_3d_diag), intent( in ) :: self !< Particle mass
    ...
    do i3 = 1, self%n_dofs(3)
       do i2 = 1, self%n_dofs(2)
          do i1 = 1, self%degree(1)
             ...
             do j3 = i3-self%degree(3), i3+self%degree(3)
                do j2 = i2-self%degree(2), i2+self%degree(2)
                   ...
                   do j1 = 1, i1+self%degree(1)
                      ind_j = j1 + modulo(j2-1,self%n_dofs(2))*self%n_dofs(1) + modulo(j3-1,self%n_dofs(3))*self%n_dofs(1)*self%n_dofs(2)
                      y(ind_i) = y(ind_i) + &
          ...
          do i1 = self%n_dofs(1)-self%degree(1)+1, self%n_dofs(1)
             ...
             do j3 = i3-self%degree(3), i3+self%degree(3)
                do j2 = i2-self%degree(2), i2+self%degree(2)
                   do j1 = i1-self%degree(1), self%n_dofs(1)
                      ind_j = j1 + modulo(j2-1,self%n_dofs(2))*self%n_dofs(1) + modulo(j3-1,self%n_dofs(3))*self%n_dofs(1)*self%n_dofs(2)
                      y(ind_i) = y(ind_i) + &
                           self%sign * self%particle_mass( ind, ind_i) * x(ind_j)
                      ind = ind +1
  ...
  end subroutine dot_particle_mass_cl_3d_diag


  subroutine print_info_particle_mass_cl_3d_diag( self )
    class(sll_t_linear_operator_particle_mass_cl_3d_diag), intent(in) :: self !< Particle mass 
  end subroutine print_info_particle_mass_cl_3d_diag
  
end module sll_m_linear_operator_particle_mass_cl_3d_diag

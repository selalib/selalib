...
program unit_test_bspline_fem_solver_1d
...
#include "sll_utilities.h"
    ...
    !!    SLL_CLEAR_ALLOCATE(equ_right_side(1),ierr)
    !!    SLL_CLEAR_ALLOCATE(solution(1:test_dimension+1),ierr)
    !
    !    call sll_bspline_fem_solver_1d_initialize(knots,5 )
    !
    !    !!SLL_DEALLOCATE_ARRAY(equ_right_side,ierr)
    !
    !
    !    call sll_bspline_fem_solver_1d_calc_inhomogenity_function(test_poisson_solver_testfunction)
    !    call sll_bspline_fem_solver_1d_solve
    !    !print *,  fem_solution
    !
    !    call sll_bspline_fem_solver_1d_eval_solution(knots, solution)
    !
    !

    !
    !    open(unit=20, file="/tmp/solution_poisson.txt")
    !    write (20,*) solution
    !    close(20)
    !!    SLL_DEALLOCATE_ARRAY(equ_right_side,ierr)
...
contains
    ...
    subroutine test_poisson_solver(test_power_two, spline_degree)
        implicit none
        integer :: ierr
        sll_real64 :: interval_a=0, interval_b=1
        sll_int, intent(in) :: test_power_two
        ...
        sll_real64 :: mesh_delta_t
        sll_real64, dimension(:), allocatable :: knots
        sll_real64, dimension(:), allocatable :: knots_eval
        ...
        knots(1)=interval_a
        do idx=2,size(knots)
            knots(idx)=interval_a  + (idx-1)*mesh_delta_t
        enddo

        !Generate Eval Mesh
        mesh_delta_t = (interval_b - interval_a)/400
        SLL_CLEAR_ALLOCATE(knots_eval(1:401),ierr)
        knots_eval(1)=interval_a
        do idx=2,size(knots_eval)
            knots_eval(idx)= interval_a + (idx-1)*mesh_delta_t
        enddo
        ...
        do  test_mode=0,test_power_two-1,1
            ...
            call set_test_mode(1.0_f64*(2**(test_mode)))

            call sll_bspline_fem_solver_1d_calc_inhomogenity_function(test_poisson_solver_testfunction,10)
            call sll_bspline_fem_solver_1d_solve


            call sll_bspline_fem_solver_1d_eval_solution(knots_eval, solution)
            actual_solution=(((2.0_f64**test_mode)*sll_kx)**(-2))*test_poisson_solver_testfunction(knots_eval)
            numerical_error(test_mode+1)=maxval(abs( (solution - actual_solution)/maxval(abs(actual_solution)) ) )

            call sll_bspline_fem_solver_1d_eval_solution_derivative(knots_eval, solution)

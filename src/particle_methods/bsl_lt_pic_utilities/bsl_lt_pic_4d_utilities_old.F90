...
module sll_bsl_lt_pic_4d_utilities_module_old_version
...
contains
  ...
  subroutine get_lt_pic_deformation_matrix (    &
                        k_part,                 &
                        particles_m2d,          &
                        p_list,                 &
...
!        sll_int32, intent(in) :: part_degree
        ...
        call cell_offset_to_global_extended(p_list(k_part)%offset_x,   &
                                            p_list(k_part)%offset_y,   &
                                            p_list(k_part)%i_cell_x,   &
                                            p_list(k_part)%i_cell_y,   &
                                            particles_m2d, x_k, y_k )
        vx_k   = p_list(k_part)%aaa
        vy_k   = p_list(k_part)%vy
        ...
        k_ngb  = p_list(k_part)%ngb_xright_index
        if( k_ngb == k_part )then
           ...
           vy_k_right  = vy_k
            ...
            call cell_offset_to_global_extended(p_list(k_ngb)%offset_x,   &
                                                p_list(k_ngb)%offset_y,   &
                                                p_list(k_ngb)%i_cell_x,   &
                                                p_list(k_ngb)%i_cell_y,   &
        ...
        k_ngb  = p_list(k_part)%ngb_xleft_index
        if( k_ngb == k_part )then
        ...
        k_ngb  = p_list(k_part)%ngb_yright_index
        if( k_ngb == k_part )then
           ! no right neighbor is available, use a non-centered finite difference
           factor = 2*factor
           x_k_right   = x_k
        ...
        k_ngb  = p_list(k_part)%ngb_yleft_index
        if( k_ngb == k_part )then
           ! no left neighbor is available, use a non-centered finite difference
           factor = 2*factor
           x_k_left   = x_k
        ...
        k_ngb  = p_list(k_part)%ngb_vxright_index
        if( k_ngb == k_part )then
           ! no right neighbor is available, use a non-centered finite difference
           factor = 2*factor
           x_k_right   = x_k
        ...
        k_ngb  = p_list(k_part)%ngb_vxleft_index
        if( k_ngb == k_part )then
            ! no left neighbor is available, use a non-centered finite difference
            factor = 2*factor
            x_k_left   = x_k
        ...
        k_ngb  = p_list(k_part)%ngb_vyright_index
        if( k_ngb == k_part )then
           ! no right neighbor is available, use a non-centered finite difference
           factor = 2*factor
           x_k_right   = x_k
        ...
        k_ngb  = p_list(k_part)%ngb_vyleft_index
        if( k_ngb == k_part )then
            ! no left neighbor is available, use a non-centered finite difference
            factor = 2*factor
            x_k_left   = x_k
...
end subroutine get_lt_pic_deformation_matrix


todo -- stop resume here
...
!    if( y > ymax ) y = y - ymax-ymin
  ...
  subroutine periodic_correction(p_group, x, y)
    type(sll_bsl_lt_pic_4d_group), pointer,  intent(in)    :: p_group
    sll_real64, intent(inout) :: x
    sll_real64, intent(inout) :: y
    type(sll_cartesian_mesh_2d), pointer :: mesh
...
!  subroutine write_lt_pic_density_on_remap_grid ( &            ! old name
  subroutine sll_lt_pic_4d_write_f_on_remap_grid ( &
    ...
    type(sll_bsl_lt_pic_4d_particle), dimension(:),  pointer :: p
...
!    open(90,file='lt_pic_density.dat')    
  ...
  subroutine plot_lt_particles( &
    ...
    do k = 1, p_group%number_particles
        ...
        call cell_offset_to_global_extended(particle%offset_x, particle%offset_y, particle%i_cell_x, particle%i_cell_y,   &
...
subroutine get_initial_position_on_cartesian_grid_from_particle_index (k,                                               &
  ...
  subroutine sll_lt_pic_4d_write_f_on_grid_or_deposit (p_group, q_accumulator,      &
    ...
    do k=1, p_group%number_particles ! [[file:../pic_particle_types/lt_pic_4d_group.F90::number_particles]]
       ...
       call cell_offset_to_global_extended(particle%offset_x,particle%offset_y,particle%i_cell_x,particle%i_cell_y,p_group%mesh,x,y)
...
;                                                                                                                       \
        if(k_neighbor /= 0) then;  do          ;                                                                        \
            ...
            call cell_offset_to_global_extended(particle%offset_x, particle%offset_y,                                   \
                                                particle%i_cell_x, particle%i_cell_y, p_group%mesh, x, y) ;             \
            call periodic_correction(p_group,x,y) ;                                                                     \
            vx = particle%vx;                                                                                           \
            vy = particle%vy;                                                                                           \
            x_aux = x - g%eta1_min;                                                                                     \
...
end module  sll_bsl_lt_pic_4d_utilities_module_old_version
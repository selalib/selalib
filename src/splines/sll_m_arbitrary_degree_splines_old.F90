...
contains
  ...
  ! find cell returns the index 'i' of the grid cell such that:
...
!!$    print*, 'find_cell:', x
!!$    do i=1,n
!!$       print*, i,spline_obj%k(i)
!!$    end do

    ! check if point is outside of grid
    if (x > spline_obj%k(n)) then
       find_cell = -1
       return
    end if
    if (x < spline_obj%k(1)) then
       find_cell = -1
       return
    end if
  ...
  do while (x < spline_obj%k(find_cell) &
       .or. x >= spline_obj%k(find_cell+1))
     if (x < spline_obj%k(find_cell)) then
        high = find_cell
     else
        low  = find_cell
     end if

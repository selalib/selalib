program sim_bsl_gc_2d0v_hex_aligned_splines
...
#include "sll_assert.h"
  ...
  do num_cells = cells_min,cells_max,cells_stp
     ...
     if (model_name.eq."GC") then
        print*, "ERROR in sim: not implemented for guiding center"
        STOP
     ...
     ! Mesh creation -------------------------
     h_mesh => new_hex_mesh_2d( num_cells, center_mesh_x1, center_mesh_x2,&
          radius=radius, EXTRA_TABLES = EXTRA_TABLES )
     t_mesh => new_triangular_mesh_2d_aligned_from_hex_mesh(h_mesh)
     call display_hex_mesh_2d(h_mesh)

     n_points   = h_mesh%num_pts_tot
     det = (h_mesh%r1_x1*h_mesh%r2_x2 - h_mesh%r1_x2*h_mesh%r2_x1)/h_mesh%delta

     r11 = + h_mesh%r2_x2 / det
     r12 = - h_mesh%r2_x1 / det
     r21 = - h_mesh%r1_x2 / det
     r22 = + h_mesh%r1_x1 / det
     ! ---------------------------------------
     ...
     spline => new_box_spline_2d(h_mesh, SLL_DIRICHLET)
     ! ---------------------------------------

     ! Initial distribution ------------------
     call init_distr_circ(rho_tn, t_mesh, &
     ...
     do while (t .lt. tmax)
        ...
        !***********************************************************************
        !                Semi-Lagrangian part (inteprolation)
        !***********************************************************************
        ...
        do i=1, n_points
           ...
           x = t_mesh%cartesian_coord(1,i)
           y = t_mesh%cartesian_coord(2,i)
           ...
           if ( abs(h1) > radius-h_mesh%delta .or. abs(h2) > radius-h_mesh%delta ) &
                inside = .false.
           if ( abs(xx) > (radius-h_mesh%delta) * sll_sqrt3 * 0.5_f64) &
                inside = .false.

           if ( inside ) then
              rho_tn1(i) = hex_interpolate_value(h_mesh, xx, yy, &
        ...
        if (model_name.eq."GC") then
           ...
           do i = 1, mesh%num_pts_tot    ! need to re-index phi :
              k1 = mesh%h_hex_coord(1, i)
              k2 = mesh%h_hex_coord(2, i)
              call mesh%index_hex_to_global(k1, k2, index_tab)
              phi(i) = phi_interm(index_tab)
           enddo

           call compute_hex_fields(h_mesh, uxn, uyn, &
        ...
        if (model_name.eq."GC") then
           ...
           if (count == 10.and.nloops<10000.and.num_cells == cells_max) then
              ...
              call t_mesh%sll_s_write_field_hex_mesh_xmf(rho_tn1, trim(filename))
              if (model_name.eq."GC") then
                 filename  = "guiding_center_phi"//trim(filenum)
                 call t_mesh%sll_s_write_field_hex_mesh_xmf(phi, trim(filename))
              end if
              count = 0
           endif
        elseif (model_name.eq."CIRCULAR") then
...
contains
  ...
  subroutine hex_diagnostics_gc(rho,t,mesh,uxn,uyn,nloop,deg,tmax,cells_min,cells_max)
    ...
    if (h_mesh%num_cells == cells_max) then
       call int2string(h_mesh%num_cells,filenum)
...
end program sim_bsl_gc_2d0v_hex_aligned_splines

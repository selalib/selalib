...
type(C_PTR) :: PlnFwd,PlnBwd,PlnF,PlnB,PlnF2 !,PlnFwdc
...
integer(4),parameter ::  ntau =64,Nn=64!, Mn=514
...
real(8)    :: k,h,gn,F1s(0:ntau-1),F2s(0:ntau-1),En(0:Ntau-1,1:Nn),Enr(0:Ntau-1,1:Nn),Ent(0:Ntau-1,1:Nn)
complex(8) :: sumup1,sumup2,dtgn
real(8)    :: tau(0:ntau-1), ltau(0:ntau-1),lx(1:Nn),Etemp(1:Nn),w1_0(0:ntau-1),w2_0(0:ntau-1),fvr(1:Nn,1:Nn)
complex(8) :: F1(0:ntau-1),F2(0:ntau-1),Ftilde1(0:ntau-1),Ftilde2(0:ntau-1)
complex(8) :: Term2(0:ntau-1),Term1(0:ntau-1),dtF1_F(0:ntau-1),dtF2_F(0:ntau-1)
complex(8) :: temp1_F(0:ntau-1),temp2_F(0:ntau-1),dtF1(0:ntau-1),dtF2(0:ntau-1)
integer(4) :: n,m
complex(8),  allocatable :: fgen1(:), fgen2(:),fgen3(:,:), fgen4(:,:)
real(8),external :: g
...
allocate (fgen1(Nn))
allocate (fgen2(Nn))
allocate (fgen3(Nn,Nn))
allocate (fgen4(Nn,Nn))
...
PlnF2= fftw_plan_dft_2d(Nn,Nn,fgen3,fgen4,FFTW_FORWARD,FFTW_MEASURE+FFTW_UNALIGNED)
deallocate (fgen1)
deallocate (fgen2)
deallocate (fgen3)
deallocate (fgen4)
...
do i=1,Neta1+1
    ...
    do j=1,Neta2+1
        ...
        !fh_fsl(i,j) =4/dsqrt(0.4d0*sll_pi)*dexp(-eta2**2/0.4d0)*0.5d0*(derf((eta1+1.2d0)/0.3d0)-derf((eta1-1.2d0)/0.3d0))
    enddo
enddo
...
m=Nn/2
lx=(/ (n, n=0,m-1), (n, n=-m,-1 )/)*2.0d0*sll_pi/(eta1_max-eta1_min)
t=0.0d0
!-------- * Evolution in time * ---------
do step=1,nb_step
    f0=fh_fsl(1:Nn,1:Nn)
    ...
    call poissonsolver(f0,r_array,v_array,tau+t,lx,Nn,Ntau,En,Enr,Ent,PlnF,PlnB,PlnF2)
    do i=1,Neta1+1
    do j=1,Neta2+1
        xi1_0=x1_array(i)
        xi2_0=x2_array(j)
    !------------for 1st order correction------------------
        do m=0,ntau-1
        Etemp=En(m,:)
        gn=g(tau(m)+t,xi1_0,xi2_0,Etemp,lx,Nn,PlnF)
    ...
    !------------for 2nd order correction------------------
        do m=0,ntau-1
        Etemp=En(m,:)
        gn=g(tau(m)+t,w1_0(m),w2_0(m),Etemp,lx,Nn,PlnF)
        ...
        Etemp=Ent(m,:)
        dtgn=g(tau(m)+t,w1_0(m),w2_0(m),Etemp,lx,Nn,PlnF)
        Etemp=Enr(m,:)
        dtgn=dtgn+g(tau(m)+t,w1_0(m),w2_0(m),Etemp,lx,Nn,PlnF)*(dcos(tau(m)+t)*Ftilde1(0)+dsin(tau(m)+t)*Ftilde2(0))
        ...
        do m=0,ntau-1
        temp1(m)=cmplx(0.0d0,0.0d0,kind=f64)
        temp2(m)=cmplx(0.0d0,0.0d0,kind=f64)
            do n=1,ntau-1
            temp1(m)=(cdexp(sll_i1*tau(m)*ltau(n))-1.0d0)*(dtF1_F(n)/sll_i1/ltau(n))+temp1(m)
            temp2(m)=(cdexp(sll_i1*tau(m)*ltau(n))-1.0d0)*(dtF2_F(n)/sll_i1/ltau(n))+temp2(m)
            enddo
        enddo
        ...
        temp2(m)=cmplx(0.0d0,0.0d0,kind=f64)
            do n=1,ntau-1
            temp1(m)=(cdexp(sll_i1*tau(m)*ltau(n))-1.0d0)*(temp1_F(n)/sll_i1/ltau(n))+temp1(m)
            temp2(m)=(cdexp(sll_i1*tau(m)*ltau(n))-1.0d0)*(temp2_F(n)/sll_i1/ltau(n))+temp2(m)
            enddo
        enddo
...
!        do m=0,ntau-1
!        Ftilde1(m)=Ftilde1(m)/(1.0d0+sll_i1*k*ltau(m)/eps)/dble(ntau)
!        Ftilde2(m)=Ftilde2(m)/(1.0d0+sll_i1*k*ltau(m)/eps)/dble(ntau)
!        enddo
!        sumup1=cmplx(0.0d0,0.0d0,kind=f64)
!        sumup2=cmplx(0.0d0,0.0d0,kind=f64)
!        do n=0,ntau-1
...
!---2nd UA
        ...
        do m=0,ntau-1
        Etemp=En(m,:)
        gn=g(tau(m)+t,dreal(AF1(m)),dreal(AF2(m)),Etemp,lx,Nn,PlnF)
        ...
        do m=0,ntau-1
        temp1(m)=(F1(m)*(1.0d0-sll_i1*k/eps/2.0d0*ltau(m))+k*Ftilde1(m))/(1.0d0+sll_i1*k/2.0d0*ltau(m)/eps)
        temp2(m)=(F2(m)*(1.0d0-sll_i1*k/eps/2.0d0*ltau(m))+k*Ftilde2(m))/(1.0d0+sll_i1*k/2.0d0*ltau(m)/eps)
        enddo
...
enddo
f0=fh_fsl(1:Nn,1:Nn)
call fvrinterp(f0,t,r_array,v_array,Nn,fvr,PlnF2)
...
subroutine fvrinterp(fh_fsl,t,r,v,Nn,fvr,PlnF2)
!------interpolation by NUFFT-------
use sll_m_constants
use, intrinsic :: iso_c_binding
implicit none
include "fftw3.f03"
type(C_PTR), intent(in)  :: PlnF2
integer(4), intent(in)   :: Nn
real(8), intent(in)      :: fh_fsl(1:Nn,1:Nn),t,r(1:Nn),v(1:Nn)!,lx(1:Nn)
real(8), intent(inout)   :: fvr(1:Nn,1:Nn)
complex(8) :: ftilde(Nn,Nn), f(1:Nn), ftemp(1:Nn,1:Nn)
complex(8),   allocatable :: ftemp_1d(:)
real(8)    :: x,y,x_array(Nn**2),y_array(Nn**2),L
integer(4) :: n,m,p,ier,ntrace(1:Nn**2),mtrace(1:Nn**2)
L=4.0d0
ftemp=fh_fsl
call  fftw_execute_dft(PlnF2, ftemp, ftilde)
ftilde=ftilde/dble(Nn**2)
...
do n=1,Nn
    ...
    y=dsin(t)*r(n)+dcos(t)*v(m)
        if (dabs(x)<L .and. dabs(y)<L) then
            ...
            ntrace(p)=n
            mtrace(p)=m
            x_array(p)=(x+L)*sll_pi/L
            y_array(p)=(y+L)*sll_pi/L
        else
            fvr(n,m)=0.0d0
        endif
    enddo
...
call nufft2d2f90(p,x_array,y_array,ftemp_1d,1,1.0d-8, Nn,Nn,ftilde,ier)
do n=1,p
...
subroutine poissonsolver(fh_fsl,r,v,tau,lx,Nn,Ntau,En,Enr,Ent,PlnF,PlnB,PlnF2)
! ---PoissonSolver-------
use sll_m_constants
use, intrinsic :: iso_c_binding
implicit none
include "fftw3.f03"
type(C_PTR), intent(in) :: PlnF,PlnB,PlnF2
integer(4), intent(in)   :: Nn,Ntau
real(8), intent(in)      :: fh_fsl(1:Nn,1:Nn),r(1:Nn),v(1:Nn),lx(1:Nn),tau(0:Ntau-1)
real(8), intent(inout)   :: En(0:Ntau-1,1:Nn),Enr(0:Ntau-1,1:Nn),Ent(0:Ntau-1,1:Nn)
real(8)    :: x(1:Nn),L,fvr(1:Nn,1:Nn),ftv(1:Nn,1:Nn),ftr(1:Nn,1:Nn)
real(8)    :: ftemp1(1:Nn,1:Nn),ftemp2(1:Nn,1:Nn),xi1,xi2
integer(4) :: n,m,i
complex(8) :: fvptilde(1:Nn),fvptilde0(1:Nn),temp(1:Nn),sum0(1:Nn)
...
x(Nn/2+1)=1.0d0
do i=0,Ntau-1
    call fvrinterp(fh_fsl,tau(i),r,v,Nn,fvr,PlnF2)
    do n=1,Nn
        do m=1,Nn
            vctmp(m) = fvr(n,m)
        enddo
...
enddo
do i=0,Ntau-1
    call fvrinterp(ftv,tau(i),r,v,Nn,ftemp1,PlnF2)
    call fvrinterp(ftr,tau(i),r,v,Nn,ftemp2,PlnF2)
    do n=1,Nn
        do m=1,Nn
        uctmp=En(i,:)
        xi1=r(n)*dcos(tau(i))-v(m)*dsin(tau(i))
        xi2=r(n)*dsin(tau(i))+v(m)*dcos(tau(i))
        vctmp(m)=(dcos(2.0d0*tau(i))**2*(xi1*dcos(tau(i))+xi2*dsin(tau(i)))+g(tau(i),xi1,xi2,dreal(uctmp),lx,Nn,PlnF))*(-dsin(tau(i))*ftemp1(n,m)+dcos(tau(i))*ftemp2(n,m))!partial_t f_tilde(xi1,xi2)
        enddo
        call fftw_execute_dft(PlnF, vctmp, fvptilde)
...
double precision function g(ta,x1,x2,E,lx,Nn,PlnF)
use sll_m_constants
use, intrinsic :: iso_c_binding
implicit none
include "fftw3.f03"
type(C_PTR),intent(in) :: PlnF
integer(4),intent(in) :: Nn
real(8),intent(in) :: x1,x2,ta,E(1:Nn),lx(1:Nn)
real(8) :: x,L
integer(4) :: n
complex(8) :: Etilde1(1:Nn),sum0, E0(1:Nn)
L=4.0d0
E0=E

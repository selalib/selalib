...
use omp_lib
  ...
  public change_equilibrium_function_va2d_par_cart
  public change_initial_function_va2d_par_cart
...
contains
  ...
  subroutine run_va2d_cartesian(sim)
    ...
    do i = 1, np_x1
       write(11,*) sim%x1_array(i), efield_0(i)
    end do
    ...
    do istep = 1, sim%num_iterations
      ...
      call advection_ampere_x(sim, layout_x1, efield_0, f_x1, 0.5_f64*sim%dt)
  ...
  subroutine advection_ampere_x(sim, layout_x1, efield, f_x1, delta_t)
    ...
    efield(1:nc_x1) = efield(1:nc_x1)*(1.0_f64 - sim%gamma_d*delta_t) / nc_x1
  ...
  subroutine diagnostics(sim, layout_x1, f_x1, rho, efield, e_app)
    ...
    potential_energy = sum(efield*efield) !/ sim%mesh2d%num_cells1

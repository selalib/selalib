...
c
c ... purpose (see muh2.d for details)
c
...
c
c     see the documentation file "muh2.d" for a complete discussion
c     of how to use subroutine muh2.  file "tmuh2.f" is a test/driver
c     sample program illustrating use of muh2
c
...
c
     ...
     +             kcycle,iprer,ipost,intpol,kps
      ...
      real(8) cof(nx,ny,6),tx(nx,ny,*),ty(ny,nx,*)
      real(8) wk(*),dlx,dlx2,dlxx,dly,dly2,dlyy,cmin,alfmax,cemax
      real(8) x,y,cxx,cyy,cx,cy,ce,c1,c2,c3,c4,c5,alfa,gbdy
      common/imud2/intl,nxa,nxb,nyc,nyd,ixp,jyq,iex,jey,nfx,nfy,iguess,
     +             maxcy,method,nwork,lwork,itero,ngrid,klevel,kcur,
     +             kcycle,iprer,ipost,intpol,kps
      common/fmud2/xa,xb,yc,yd,tolmax,relmax
...
c
      implicit none
      integer nx,ny,nxa
      real(8) cof(nx,ny,6),beta(nx,nx,*),alfa(nx,nx,*)
      integer index(nx,ny)
...
c
      ...
      real(8) phi(0:nx+1,0:ny+1),cof(nx,ny,6)
...
c
      implicit none
      integer nx,ny,index(nx,ny)
      real(8) phi(0:nx+1,0:ny+1),cof(nx,ny,6),beta(nx,nx,*)
      integer iz,j,jb,i
      iz = 0
      ...
      real(8) cof(nx,ny,6),alfa(nx,nx,*),beta(nx,nx,*)
      real(8) dmat(nx,nx,*),zmat(nx,nx,*),sum
      jcur = 1
c
c     set dmat(1)=alfa(1)
c
...
c     backward sweep
      ...
      real(8) phi(0:nx+1,0:ny+1),frhs(nx,ny),sum
...
c
      ...
      real(8) cof(nx,ny,6),beta(nx,nx,ny),dmat(nx,nx,*)
      real(8) sum
      iz = 0
...
c
     ...
     +             kcycle,iprer,ipost,intpol,kps
      ...
      real(8) phi(0:nx+1,0:ny+1),cof(nx,ny,6)
      real(8) dlx,dlx2,dlxx,dly,dly2,dlyy
      real(8) x,y,cxx,cyy,cx,cy,ce,c1,c2,c3,c4,alfa,gbdy
      common/imud2/intl,nxa,nxb,nyc,nyd,ixp,jyq,iex,jey,nfx,nfy,iguess,
     +             maxcy,method,nwork,lwork,itero,ngrid,klevel,kcur,
     +             kcycle,iprer,ipost,intpol,kps
      common/fmud2/xa,xb,yc,yd,tolmax,relmax
...
c
     ...
     +             maxcy,method,nwork,lwork,itero,ngrid,klevel,kcur,
     +             kcycle,iprer,ipost,intpol,kps
      real(8) phi(0:nx+1,0:ny+1),cof(nx,ny,6),tx(nx,ny,*),sum(ny)
c
...
c
     ...
     +             maxcy,method,nwork,lwork,itero,ngrid,klevel,kcur,
     +             kcycle,iprer,ipost,intpol,kps
      real(8) phi(0:nx+1,0:ny+1),cof(nx,ny,6),ty(ny,nx,*),sum(nx)
c

...
sll_int32, parameter :: KNOTS_PERIODIC  = 0
!> PLEASE ADD DOCUMENTATION
sll_int32, parameter :: KNOTS_DIRICHLET = 1

contains

subroutine initialize_knots_source( spline_degree, &
                                    num_cells,     &
                                    eta_min,       &
                                    eta_max,       &
                                    knots)
...
if (mod(spline_degree + 1, 2) == 0) then
 do i = spline_degree + 2, num_cells + 1
   knots(i) = eta_min + (i - (spline_degree +1)/2 - 1) * delta  
 end do
else
 do i = spline_degree + 2, num_cells + 1

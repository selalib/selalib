...
!subroutine write_xmf_file(mesh, array, array_name, iplot)
!
! use hdf5
! use sll_hdf5_io_serial
!
...
! call write_fx1x2(mesh,cplot)
! call write_fx1x3(mesh,cplot)
! call write_fx2x4(mesh,cplot)
! call write_fx3x4(mesh,cplot)
!
!    nx1 = mesh%num_cells1
!    nx2 = mesh%num_cells2
!    nx3 = mesh%num_cells3
...
!    call write_grid(mesh,file_id,nx1,nx2,"x1","x2",cplot)
!    call write_grid(mesh,file_id,nx1,nx3,"x1","x3",cplot)
!    call write_grid(mesh,file_id,nx2,nx4,"x2","x4",cplot)
!    call write_grid(mesh,file_id,nx3,nx4,"x3","x4",cplot)
...
!
!subroutine write_grid(mesh,file_id,nx,ny,xname,yname,cplot)
!
! class(sll_cartesian_mesh_4d),intent(in) :: mesh
...
!
!subroutine write_fx1x2(mesh,cplot)
!use sll_hdf5_io_serial
!class(sll_cartesian_mesh_4d),intent(in)     :: mesh
!character(len=*)                    :: cplot
!sll_int32                           :: error
...
!call compute_local_sizes(mesh%layout_p,loc_sz_i,loc_sz_j,loc_sz_k,loc_sz_l)        
!SLL_CLEAR_ALLOCATE(fij(1:loc_sz_i,1:loc_sz_j),error)
!do j=1,loc_sz_j
!   do i=1,loc_sz_i
!      sumloc = sum(mesh%f(i,j,:,:))
...
!
!subroutine write_fx1x3(mesh,cplot)
!use sll_hdf5_io_serial
!class(sll_cartesian_mesh_4d),intent(in)     :: mesh
!character(len=*)                    :: cplot
!sll_int32                           :: error
...
!
!subroutine write_fx2x4(mesh,cplot)
!use hdf5
...
!do l=1,loc_sz_l
!   do j=1,loc_sz_j
!      fjl(j,l) = sum(mesh%f(:,j,:,l))
!   end do
!end do
!global_dims = (/mesh%nc_eta2,mesh%nc_eta4/)
!offset(1) = get_layout_j_min(mesh%layout_p,prank)-1
!offset(2) = get_layout_l_min(mesh%layout_p,prank)-1
...
!
!subroutine write_fx3x4(mesh,cplot)
!use hdf5
!use sll_hdf5_io_parallel
!class(sll_cartesian_mesh_4d),intent(in)     :: mesh
!character(len=*)                    :: cplot
...
!do l=1,loc_sz_l
!   do k=1,loc_sz_k
!      fkl(k,l) = sum(mesh%f(:,:,k,l))
!   end do
!end do
!global_dims = (/mesh%nc_eta3,mesh%nc_eta4/)
!offset(1) = get_layout_k_min(mesh%layout_p,prank)-1
!offset(2) = get_layout_l_min(mesh%layout_p,prank)-1

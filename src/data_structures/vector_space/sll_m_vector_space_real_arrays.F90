...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  ...
  !============================================================================
  type, abstract, extends( sll_c_vector_space ) :: sll_c_vector_space_real

    real(wp), private, pointer, contiguous :: d(:)        => null()
  ...
  end type sll_c_vector_space_real

  !============================================================================
  ! Error messages
  !============================================================================
  character( len=* ), parameter :: WRONG_TYPE_X = &
    "x not of class( sll_c_vector_space_real )"

  character( len=* ), parameter :: WRONG_TYPE_Y = &
    "y not of class( sll_c_vector_space_real )"
  ...
  !============================================================================
  type, extends( sll_c_vector_space_real ) :: sll_t_vector_space_real_1d

    real(wp), pointer, contiguous :: array(:) => null()      !< 1D real array

  contains
    procedure          :: attach          => attach__real_1d !< Attach 1D array
  ...
  !============================================================================
  type, extends( sll_c_vector_space_real ) :: sll_t_vector_space_real_2d

    real(wp), pointer, contiguous :: array(:,:) => null()    !< 2D real array
  ...
  !============================================================================
  type, extends( sll_c_vector_space_real ) :: sll_t_vector_space_real_3d

    real(wp), pointer, contiguous :: array(:,:,:) => null()  !< 3D real array
  ...
  end type sll_t_vector_space_real_3d
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  ...
  subroutine copy__real( self, x )
    class( sll_c_vector_space_real ), intent( inout ) :: self
    class( sll_c_vector_space      ), intent( in    ) :: x
    select type( x ); class is( sll_c_vector_space_real )
  ...
  subroutine add__real( self, x, y )
    ...
    select type( x ); class is( sll_c_vector_space_real )
    select type( y ); class is( sll_c_vector_space_real )
...
!    end do
  ...
  function norm__real( self ) result( res )
    class( sll_c_vector_space_real ), intent( in ) :: self
    real(wp) :: res
  ...
  subroutine initialize_copy__real_1d( self )
    ...
    real(wp), pointer, contiguous :: p(:)
    integer                       :: sz, lb(1), ub(1)
  ...
  subroutine attach__real_2d( self, x )
    ...
    real(wp),                contiguous, target, intent( inout ) :: x(:,:)
    ! Safer option: Automatic pointer targetting, not yet supported by ifort:
    ! real(wp) , contiguous, pointer, intent( in ) :: x(:,:)
  ...
  subroutine attach__real_3d( self, x )
    class( sll_t_vector_space_real_3d ),         intent( inout ) :: self
    real(wp),             contiguous , target, intent( inout ) :: x(:,:,:)
    ! Safer option: Automatic pointer targetting, not yet supported by ifort:
    ! real(wp) , contiguous, pointer, intent( in ) :: x(:,:,:)

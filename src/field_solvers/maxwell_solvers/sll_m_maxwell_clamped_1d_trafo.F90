...
#include "sll_working_precision.h"
  ...
  use sll_m_linear_operator_poisson_clamped_1d, only : &
       sll_t_linear_operator_poisson_clamped_1d
  ...
  type, extends(sll_c_maxwell_1d_base) :: sll_t_maxwell_clamped_1d_trafo
...
contains
  ...
  subroutine L2projection_1d_trafo(self, func, degree, coefs_dofs)
    ...
    if (degree == self%s_deg_0) then
       ...
       call sll_s_compute_rhs_trafo(self, func, degree, self%work0)
       call self%invert_mass( self%work0, coefs_dofs, self%s_deg_0 )
    elseif  (degree == self%s_deg_1) then
       ! Compute right-hand-side
       call sll_s_compute_rhs_trafo(self, func, degree, self%work1)
       call self%invert_mass( self%work1, coefs_dofs, self%s_deg_1 )
    else
       print*, 'degree ', degree, 'not availlable in maxwell_clamped_1d_trafo object' 
    endif
  ...
  subroutine init_1d_trafo( self, domain, n_cells, s_deg_0, boundary, map, mass_tolerance, poisson_tolerance, solver_tolerance )
    ...
    ! Sparse matrices
    call sll_s_spline_fem_mass1d_clamped_full ( n_cells, self%s_deg_0, self%spline0_pp, self%mass0, profile_0 )
    call sll_s_spline_fem_mass1d_clamped_full ( n_cells, self%s_deg_1, self%spline1_pp, self%mass1, profile_1 )
    ...
    call sll_s_spline_fem_mixedmass1d_clamped_full( n_cells,  self%s_deg_0,  self%s_deg_1, self%mixed_mass, profile_m0, self%spline0_pp, self%spline1_pp )
  ...
  subroutine init_from_file_1d_trafo( self, domain, n_cells, s_deg_0, boundary, map, nml_file )
    ...
    if (io_stat /= 0) then
       if (rank == 0 ) then
          print*, 'sll_m_maxwell_clamped_1d_trafo: Input file does not exist. Set default tolerance.'
       ...
       if (io_stat /= 0 .or. io_stat1 /= 0 ) then
          if (rank == 0 ) then
             print*, 'sll_m_maxwell_clamped_1d_trafo: Input parameter does not exist. Set default tolerance.'
             open(newunit=file_id, file=trim(file_prefix)//'_parameters_used.dat', position = 'append', status='old', action='write', iostat=io_stat)
             write(file_id, *) 'mass solver tolerance:', 1d-12
             write(file_id, *) 'poisson solver tolerance:', 1d-12
             close(file_id)
  ...
  subroutine multiply_mass_1d_trafo( self,  in, out, degree)
    ...
    elseif(degree == 10) then
       ...
       print*, 'maxwell_solver_clamped_1d_trafo: multiply mass for other form not yet implemented'
       stop
    end if
...
end module sll_m_maxwell_clamped_1d_trafo

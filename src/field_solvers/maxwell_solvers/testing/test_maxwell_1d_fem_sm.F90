...
!
program test_maxwell_1d_fem_sm
  !------------------------------------------------------------------------
...
#include "sll_maxwell_solvers_macros.h"
  ...
  type(sll_t_maxwell_1d_fem_sm)  :: maxwell_1d
  ...
  call maxwell_1d%init(domain, nc_eta1, deg, dt, mass_tolerance=1d-12, poisson_tolerance=1d-12)
  ...
  if ((err_bz < tol) .and. (err_ey < tol) .and. (err_ex < 2.0d-7) .and. (err_ex2 < 1.0d-8) .and. (err_l2norm < 1.0d-13).and. (err_inner < 1.0d-10)) then
     print*,'PASSED'
  endif
...
end program test_maxwell_1d_fem_sm

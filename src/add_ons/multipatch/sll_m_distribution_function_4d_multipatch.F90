...
!------------------------------------------------------------------------------
!
! MODULE: sll_t_distribution_function_4d_multipatch
!
!> @author
!> - Edwin
!
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  ...
  type ::  sll_t_distribution_function_4d_multipatch
     ...
     type(sll_t_cartesian_mesh_2d), pointer :: mesh_v ! same for all patches
     type(sll_t_coordinate_transformation_multipatch_2d), pointer :: transf
     type(sll_t_layout_4d_ptr), dimension(:), pointer :: layouts_x1x2
     type(sll_t_layout_4d_ptr), dimension(:), pointer :: layouts_x3x4
     type(sll_t_layout_2d_ptr), dimension(:), pointer :: layouts_split
     type(sll_t_layout_2d_ptr), dimension(:), pointer :: layouts_full
     type(data_4d_ptr), dimension(:), pointer :: f_x1x2
     type(data_4d_ptr), dimension(:), pointer :: f_x3x4
     type(sll_t_multipatch_data_2d_real), dimension(:), pointer :: rho_split
     type(sll_t_remap_plan_2d_real64_ptr), dimension(:), pointer :: remap_split2full
     type(sll_t_remap_plan_4d_real64_ptr), dimension(:), pointer :: remap_x1x2tox3x4
     type(sll_t_remap_plan_4d_real64_ptr), dimension(:), pointer :: remap_x3x4tox1x2
...
contains
  ...
  function sll_f_new_distribution_function_4d_multipatch( &
       collective, &
       transf_mp, &
    ...
    type(sll_t_collective_t), pointer :: collective
    type(sll_t_coordinate_transformation_multipatch_2d), intent(in), target:: &
         transf_mp
    type(sll_t_cartesian_mesh_2d), pointer :: mesh_v
    ...
    num_proc_total = sll_f_get_collective_size(collective)
    myrank = sll_f_get_collective_rank(collective)
  ...
  end function sll_f_new_distribution_function_4d_multipatch
  ...
  subroutine allocate_memory_df_4d_mp( df )
    ...
    myrank = sll_f_get_collective_rank(df%collective)
    col_size = sll_f_get_collective_size(df%collective)
    ...
    do ip=0,df%num_patches-1
       ...
       df%layouts_x1x2(ip+1)%l => sll_f_new_layout_4d(df%collective)
       ...
       df%layouts_x3x4(ip+1)%l => sll_f_new_layout_4d(df%collective)
       call sll_o_initialize_layout_with_distributed_array( &
            np_x1, &
            np_x2, &
            np_x3, &
       ...
       df%layouts_split(ip+1)%l => &
            sll_f_new_layout_2d_from_layout_4d( df%layouts_x3x4(ip+1)%l )

       df%layouts_full(ip+1)%l => sll_f_new_layout_2d(df%collective)
       ...
       do j=0,col_size-1
          call sll_o_set_layout_i_min( df%layouts_full(ip+1)%l, j, 1)
          call sll_o_set_layout_j_min( df%layouts_full(ip+1)%l, j, 1)
          call sll_o_set_layout_i_max( df%layouts_full(ip+1)%l, j, imax)
          call sll_o_set_layout_j_max( df%layouts_full(ip+1)%l, j, jmax)
       end do

       ! call sll_o_view_lims(df%layouts_split(ip+1)%l)
       ! call sll_o_view_lims(df%layouts_full(ip+1)%l)

       df%remap_split2full(ip+1)%r => &
            sll_o_new_remap_plan( df%layouts_split(ip+1)%l, &
                            df%layouts_full(ip+1)%l, &
                            df%rho_split(ip+1)%array )

       df%remap_x1x2tox3x4(ip+1)%r => &
            sll_o_new_remap_plan( df%layouts_x1x2(ip+1)%l, &
                            df%layouts_x3x4(ip+1)%l, &
                            df%f_x1x2(ip+1)%f )

       df%remap_x3x4tox1x2(ip+1)%r => &
            sll_o_new_remap_plan( df%layouts_x3x4(ip+1)%l, &
                            df%layouts_x1x2(ip+1)%l, &
                            df%f_x3x4(ip+1)%f )
  ...
  function get_x1x2_slice_4d( df, patch, k, l ) result(ptr)
    ...
    class(sll_t_distribution_function_4d_multipatch), intent(in) :: df
  ...
  subroutine sll_s_compute_charge_density_multipatch( df, rho )
    type(sll_t_distribution_function_4d_multipatch), intent(in) :: df
    class(sll_t_scalar_field_multipatch_2d), intent(inout)      :: rho
    sll_real64 :: delta3
    sll_real64 :: delta4
    sll_int32  :: ipatch
    sll_int32  :: i
    ...
    do ipatch=0, num_patches - 1
       call sll_o_compute_local_sizes( df%layouts_split(ipatch+1)%l, locsz1, locsz2 )
       do j=1, locsz2
          do i=1, locsz1
       ...
       call sll_o_apply_remap_2d( &
            df%remap_split2full(ipatch+1)%r, &
            df%rho_split(ipatch+1)%array, &
            remap_out )
    end do

  end subroutine sll_s_compute_charge_density_multipatch
  ...
  function mass_df4d( df ) result(mass)
    ...
    call sll_o_collective_allreduce(df%collective, send_b, 1, MPI_SUM, recv_b)
    mass = recv_b(1)
  ...
  function Lp_norms_df4d( df ) result(Lp_norms)
    ...
    call sll_o_collective_allreduce(df%collective, send_b, 3, MPI_SUM, recv_b)
    Lp_norms(:) = recv_b(:)
  ...
  subroutine delete_df_4d_mp( df )
    ...
    do i = 0,num_patches-1
       ...
       call sll_o_delete( df%layouts_x1x2(i+1)%l )
       call sll_o_delete( df%layouts_x3x4(i+1)%l )
       call sll_o_delete( df%layouts_split(i+1)%l )
       call sll_o_delete( df%layouts_full(i+1)%l )
       call sll_o_delete( df%remap_split2full(i+1)%r )
       call sll_o_delete( df%remap_x1x2tox3x4(i+1)%r )
       call sll_o_delete( df%remap_x3x4tox1x2(i+1)%r )
    end do

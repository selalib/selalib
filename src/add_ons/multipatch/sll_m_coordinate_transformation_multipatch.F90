...
!> The object's resources can be liberated by a call to 'sll_o_delete()'.
...
!     character(len=128), dimension(:), pointer :: patch_names => null()
 ...
 !    type(sll_cartesian_mesh_multipatch_2d), pointer :: logical_mesh_mp => null()
     type(sll_t_coordinate_transformation_2d_nurbs_ptr), dimension(:), pointer::&
          transfs => null()
...
!    SLL_ALLOCATE(mp%patch_names(number_patches),ierr)
    ...
    do i=1,number_patches
       mp%transfs(i)%T => &
            sll_f_new_nurbs_2d_transformation_from_file(trim(patches(i)))
    end do

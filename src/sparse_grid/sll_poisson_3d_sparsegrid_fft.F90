module sll_poisson_3d_sparsegrid_fft
...
type,public:: sll_fft3d_derivative
   sll_real64,dimension(:),pointer :: kx, ky, kz
   sll_real64,dimension(:),pointer :: kpot, kex, key, kez
   sll_comp64, dimension(:),pointer :: fcoeffs, fcoeffs2

contains
   procedure,pass(this) :: initialize=>new_poisson_3d_sparsegrid_fft
...
subroutine new_poisson_3d_sparsegrid_fft(this,interpolator)
  class(sll_fft3d_derivative),intent(inout) ::this
  type(sparse_grid_interpolator_3d),intent(in) ::interpolator
  ...
  do i2 = 0,interpolator%levels(2)
     !do i3 = max(0,interpolator%max_level-i2-interpolator%levels(1)),min(interpolator%max_level - i2,interpolator%levels(3))
  ...
  do i1 = 0,interpolator%levels(1)
     !do i3 = max(0,interpolator%max_level-i1-interpolator%levels(2)),min(interpolator%max_level - i1,interpolator%levels(3))
  ...
  do i1 = 0,interpolator%levels(1)
     !do i2 = max(0,interpolator%max_level-i1-interpolator%levels(3)),min(interpolator%max_level - i1,interpolator%levels(2))
...
end subroutine new_poisson_3d_sparsegrid_fft
...
subroutine solve_for_electric_field(this,interpolator,rho,ex,ey,ez)
  class(sll_fft3d_derivative),intent(inout) ::this
  type(sparse_grid_interpolator_3d), intent(inout)   :: interpolator
  sll_real64,dimension(:),intent(inout) ::ex,ey,ez
  sll_real64,dimension(:),intent(inout) ::rho
  sll_int32 :: i
...
call ISPFFT(interpolator,this%fcoeffs2,ez)
...
end module sll_poisson_3d_sparsegrid_fft
